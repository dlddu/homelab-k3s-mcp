...
**「재기동 불필요」는 파드 정체로 잰다** — 처음과 끝에 서버 파드의 정체를 떠서 같은지 보고,
폴링 창 전체를 덮는 단언이라 재기동이 끼어들 자리가 없다. 파드 uid 만으로는 부족해 컨테이너
재시작까지 함께 뜬다(캐시는 프로세스 안에 있어 파드가 살아 있어도 컨테이너가 죽으면 비워진다).
