...
Go 단위 ``internal/mcp/delete_collection_test.go`` 여섯이 이 도구의 계약을 이미
단언한다(0건 승인 생략 · context 의 대상 수와 이름 · 20개 절단 · 승인 뒤 증가 거부 ·
namespace 누락 거부 · 셀렉터 무변형 전달). 그 여섯은 전부 **가짜 목록 판독기와 가짜
게이트** 위에서 돈다. 이 파일이 재는 것은 그 자리의 실물이다 — 실 apiserver 의
``PartialObjectMetadataList`` 목록이 승인 화면의 숫자와 이름이 되는지, 실물 gatekeeper
가 그 화면을 사람에게 그대로 보여 주는지, 그리고 승인 뒤 apiserver 로 나간
deletecollection 이 **겨냥한 것만** 지우는지.

「다른 레이블 2개는 남는다」가 이 층에서만 관측된다는 점이 이 파일의 존재 이유다.
...
각 그룹은 케이스 직전에 **레이블째 지우고 다시 세운다**. 마지막 케이스가 요구하는 것은
「승인 시점에 정확히 2개」이고, 늦게 오는 3번째가 이미 남아 있으면 ``kubectl apply`` 는
아무것도 바꾸지 않아 **선택이 늘지 않는다** — 거부를 기대한 자리가 조용히 성공으로
통과한다. 재실행 가능성을 위한 청소가 아니라 그 자리의 판별식을 세우는 장치다.

``병렬 레인:`` 을 선언하지 않는 것은 의도다. 러너는 레인 없는 파일을 레인 단계가 끝난
뒤 단독으로 돌리므로(``run_all.py`` 머리말), 이 파일이 ``workload-test`` 에서 레이블로
지우는 동안 같은 네임스페이스를 보는 다른 파일이 돌지 않는다.
"""
