...
구조 단언을 하나 더 얹은 이유: 크기만 재면 **응답이 객체를 담은 채 그냥 작을 수도** 있다.
그래서 행의 모든 칸이 스칼라인지(= 중첩 객체가 실려 오지 않았는지)와 `managedFields` 문자열이
응답 어디에도 없는지를 함께 본다. AC17 이 list 를 게이트 밖에 두는 근거가 바로 「표에는 값이
실리지 않는다」이므로, 이 축은 크기가 아니라 형태로 지켜져야 한다.
"""
