...
가짜 연결이 구별하지 못하는 것이 하나 있고 그것이 이 파일의 존재 이유다: 가짜는 자기가 받은
바이트를 기록하고 정해진 답을 돌려줄 뿐이라, **쓰기 반쪽을 닫는 구현과 닫지 않는 구현이 둘 다
통과한다.** 닫지 않으면 EOF 까지 읽는 서버(포트 너머의 거의 전부가 그렇다)는 답을 시작조차
하지 않고, 그 차이는 실물 서버 앞에서만 나타난다 — 여기서는 읽기 창을 다 쓴 빈 응답으로.

**대상 파드는 이 파일이 스스로 세운다.** 기존 픽스처에 HTTP 를 서빙하는 파드가 없고(등재
문서가 이 행을 「이 렌즈 소관의 저작」으로 적은 자리다), 픽스처 YAML 과 ``ci.yml`` 을 늘리는
대신 자매 ``resource_generic_ac12.py`` 가 ConfigMap 을 세우는 것과 같은 방식으로 파드 하나를
세운다. 러너가 파일을 자동 발견하므로 배선도 늘지 않는다. 이미지는 이미 클러스터에 있는
``busybox:1.36`` 이라 새 pull 이 없다(``resource-generic-fixture.yaml`` 의 `chatty` 와 같은 것).

**두 응답 파일을 미리 만들어 둔다** — 작은 것과 상한을 넘기는 것. 상한 케이스를 「끝없이 뱉는
서버」로 세우지 않는 것은 의도다: 그러면 잘림이 상한 때문인지 읽기 창이 만료해서인지 구별되지
않고, 시나리오가 요구하는 것은 **상한 초과가 잘린다**는 쪽이다. 정확히 상한만큼 돌아오는
응답은 창이 아니라 상한이 끊었다는 증거다.

**「같은 승인 id 로 두 번째 왕복」을 이 층에서 어떻게 재는가.** 승인 id 는 게이트가 발급하고
호출자가 고르지 못하므로, 클라이언트가 「같은 id 를 다시 쓴다」를 직접 시도할 길은 없다. 이
층에서 관측 가능한 형태는 그 부정형이다 — 같은 호출을 한 번 더 하면 **새 승인 요청이 새 id 로
생기고 호출은 다시 막힌다**(첫 승인이 터널을 열어 둔 채 재사용되지 않는다). 그 새 요청을
