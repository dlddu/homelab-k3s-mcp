...
def can_i(verb: str, resource: str, namespace: str | None = None) -> str:
    ...
    not a re-reading of k8s/rbac.yaml.
    """
    cmd = [
        "kubectl",
        "auth",
    ...
    assert answer in {"yes", "no"}, (
        f"unexpected `kubectl auth can-i {verb} {resource}` output:"
...
def test_platform_auth_safety_ac3_rbac_boundary() -> None:
    ...
    Asks the apiserver, through SubjectAccessReview, what the server's
    ServiceAccount may do. The allowed set mirrors k8s/rbac.yaml (workloads
    get/list/watch/patch, pods get/list, pods/log get, pods/exec get+create,
    namespaces and events get/list); the denied set is the AC's explicit
    exclusion list (workload delete/create and secret reads) plus pod deletion.

    This is a runtime observation of the RBAC that is actually bound in the
    cluster, not a re-reading of the manifest: an over-broad rule added
    elsewhere (another ClusterRoleBinding, a group grant) would flip a denied
    probe to "yes" here while k8s/rbac.yaml still looked clean.
    """
    allowed = [
        ("get", "deployments.apps", NAMESPACE),
        ("list", "deployments.apps", NAMESPACE),
        ("watch", "deployments.apps", NAMESPACE),
        ("patch", "deployments.apps", NAMESPACE),
        ("get", "statefulsets.apps", NAMESPACE),
        ("patch", "daemonsets.apps", NAMESPACE),
        ("get", "pods", NAMESPACE),
        ("list", "pods", NAMESPACE),
        ("get", "pods/log", NAMESPACE),
        ("get", "pods/exec", EXEC_NAMESPACE),
        ("create", "pods/exec", EXEC_NAMESPACE),
        ("get", "namespaces", None),
        ("list", "namespaces", None),
        ("get", "events", NAMESPACE),
        ("list", "events", NAMESPACE),
    ]
    denied = [
