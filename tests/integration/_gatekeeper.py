...
# --- 결함 주입기 (시나리오 5 의 409·5xx) -------------------------------------
#
# ``tests/k8s/kind/gatekeeper-injector.yaml`` 의 제어 API. 주입은 실물 gatekeeper
# 의 SQLite 저장소에 **마커 한정** ``BEFORE INSERT`` 트리거를 걸어 409·500 을 실물
# 핸들러가 스스로 내게 하는 것이고, 허용 범위는 ``docs/e2e-mocking-policy.md`` 의
# 「실환경 주입 판정」 조건 C1~C5 다. 이 헬퍼를 쓰는 파일은 그 조건 안에 머문다 —
# 특히 **arm 한 것은 같은 테스트가 disarm 하고 잔여 0 을 스스로 확인한다**(C4).
...
def injector_url() -> Iterator[str]:
    ...
    ``gatekeeper_url`` 과 같은 이유로 로컬 포트를 매번 새로 고른다.
    """
    with port_forward(
        GATEKEEPER_NAMESPACE,
        INJECTOR_SERVICE,
...
def arm(url: str, mode: str, marker: str) -> dict:
    ...
    것은 「트리거가 이 마커에 실제로 발화한다」가 관측됐다는 뜻이다 — 발화하지 않는
    트리거 위에서 공허하게 통과하는 단언을 막는다.
    """
    response = httpx.post(
...
def disarm(url: str) -> dict:
    """트리거를 걷고 그림자 행을 지운다. 잔여 0 을 돌려준다(C4)."""
