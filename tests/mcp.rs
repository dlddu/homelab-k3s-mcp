...
use homelab_k3s_mcp::grafana::{GrafanaCloudService, GrafanaCloudToken, GrafanaError};
...
fn unavailable_grafana() -> Arc<dyn GrafanaCloudService> {
...
struct FakeGrafana {
    pub calls: Mutex<u32>,
    pub response: Mutex<Option<Result<GrafanaCloudToken, GrafanaError>>>,
}

#[async_trait]
impl GrafanaCloudService for FakeGrafana {
    async fn create_short_lived_token(&self) -> Result<GrafanaCloudToken, GrafanaError> {
        *self.calls.lock().unwrap() += 1;
        match self.response.lock().unwrap().take() {
            Some(Ok(token)) => Ok(token),
            Some(Err(err)) => Err(err),
            None => Ok(GrafanaCloudToken {
                token: "glc_fake".into(),
                expires_at: "2026-05-24T01:00:00Z".into(),
                access_policy_id: "ap-fake".into(),
            }),
        }
    }
}
...
async fn tools_list_includes_workload_tools() {
    ...
    assert!(names.contains(&"grafana_cloud_token"));
}
...
async fn github_app_installation_token_dispatches_with_defaults() {
    ...
    *fake.response.lock().unwrap() = Some(Ok(InstallationToken {
    ...
    let app = homelab_k3s_mcp::app(None, unavailable_k8s(), fake.clone(), unavailable_grafana());
...
#[tokio::test]
async fn tools_list_advertises_grafana_cloud_token() {
    let response = homelab_k3s_mcp::app(
        None,
        unavailable_k8s(),
        unavailable_github(),
    ...
    assert!(tools
        .iter()
        .any(|t| t["name"].as_str() == Some("grafana_cloud_token")));

    let token = find_tool(tools, "grafana_cloud_token");
    assert!(
        token["inputSchema"]["required"].is_null()
            || token["inputSchema"]["required"]
                .as_array()
    ...
    assert_eq!(token["annotations"]["title"], "Grafana Cloud Token");
    assert_eq!(token["annotations"]["readOnlyHint"], false);
    assert_eq!(token["annotations"]["destructiveHint"], false);
    assert_eq!(token["annotations"]["idempotentHint"], false);
    assert_eq!(token["annotations"]["openWorldHint"], true);
...
#[tokio::test]
async fn grafana_cloud_token_dispatches_and_renders_env_resource() {
    let fake = Arc::new(FakeGrafana::default());
    *fake.response.lock().unwrap() = Some(Ok(GrafanaCloudToken {
        token: "glc_short_lived".into(),
        expires_at: "2026-05-24T01:00:00Z".into(),
        access_policy_id: "ap-12345".into(),
    }));
    ...
    assert_eq!(resource["type"], "resource");
    assert_eq!(resource["resource"]["mimeType"], "text/plain");
    let uri = resource["resource"]["uri"].as_str().expect("resource uri");
    assert!(uri.ends_with(".env"), "uri should be an env file: {uri}");
    let text = resource["resource"]["text"]
        .as_str()
        .expect("resource text");
    assert!(
        text.contains("GRAFANA_CLOUD_TOKEN=glc_short_lived"),
        "{text}"
    );
    assert!(
        text.contains("# Expires at: 2026-05-24T01:00:00Z"),
        "{text}"
    );
    assert!(text.contains("# Access policy: ap-12345"), "{text}");

    assert_eq!(*fake.calls.lock().unwrap(), 1);
}

#[tokio::test]
async fn grafana_cloud_token_dispatches_without_arguments_field() {
    let fake = Arc::new(FakeGrafana::default());
    let app = homelab_k3s_mcp::app(None, unavailable_k8s(), unavailable_github(), fake.clone());
    ...
    let response = app
        .oneshot(json_request(
            ...
            json!({
                ...
                "id": 102,
                "method": "tools/call",
                "params": { "name": "grafana_cloud_token" }
            }),
        ))
        .await
        .unwrap();
...
#[tokio::test]
async fn grafana_cloud_token_unavailable_returns_tool_error() {
    let response = homelab_k3s_mcp::app(
        None,
        unavailable_k8s(),
        unavailable_github(),
