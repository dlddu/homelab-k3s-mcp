...
- **설명**: `resource_proxy`는 apiserver를 거쳐 대상의 HTTP 엔드포인트에 도달하며,
  ...
  **경로 제한을 두지 않는다.** 모든 메서드와 모든 경로가 게이트를 탄다.
  ...
  `Pod`·`Service`는 선택 인자 `port`로 **도달할 포트**를 고른다. 번호(1–65535)를 받고,
  `Service`는 서비스 포트 이름도 받는다 — apiserver는 `Service` 프록시에서만 포트 이름을 번호로
  풀고 `Pod` 프록시에서는 이름을 그대로 `⟨파드 IP⟩:⟨이름⟩`에 붙여 실패하므로, `Pod`의 이름
  포트는 **승인 요청 전에** 거부한다(2026-09-22 운영에서 관측). 생략하면 apiserver 기본(첫
  번째로 선언된 포트)이 쓰인다. apiserver 프록시가 포트를
  `⟨이름⟩:⟨포트⟩`로 표기하지만 이 도구는 그 표기를 **`name`에 받지 않는다** — `name`은 게이트가
  대상을 읽는 좌표이기도 해서, 포트가 섞이면 존재하지 않는 객체를 찾게 된다. 그래서 `name`에
  `:`가 있으면 `port`를 쓰라는 안내와 함께 **승인 요청 전에** 거부한다. `Node`는 `port`를
  받지 않는다 — `nodes/proxy`는 kubelet에 닿는 경로이고 고권한 표시도 그것을 전제로 한다.
  고른 포트는 승인 `context`의 경로 옆에 실리고 응답의 `port`로도 돌아온다.
- **달성 가치**: V1, V3
- **검증 방법**: `Pod`·`Service`·`Node` 대상의 `GET`·`POST`가 동작하고 각각 `get`·`create`
  ...
  **실행되되** 그 승인 요청 `context`에 경로 전문과 고권한 표시가 함께 담긴다.
  ...
  표시된다. `port`를 준 `Pod`·`Service` 호출이 그 포트로 가고 승인 `context`와 응답에 포트가
  실리며, `name`에 포트를 붙인 호출·`Node`의 `port`·`Pod`의 이름 포트·범위 밖 포트가 **클러스터 호출 0·승인
  요청 0**으로 거부된다.
