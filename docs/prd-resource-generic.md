...
### AC14: 포트 포워드
- **설명**: `resource_port_forward`는 `create` on `pods/portforward`만 행사한다. **승인 하나는
  지속 세션 하나를 연다** — 승인 뒤 한 번의 `create`로 터널을 열고, 그 터널을 세션으로
  유지하며 `sessionId`를 돌려준다. 이후 호출은 `sessionId`와 페이로드를 받아 **같은 터널
  위에서** 보내고 읽으며, 새 승인을 받지 않는다. 쿠버네티스 API 호출은 여는 `create` 한
  번뿐이므로 1승인 1실행(`prd-approval-gate` AC7)과 어긋나지 않는다.

  세션은 **한 번만 열린다**. 닫히면(명시적 종료, 유휴 만료, 수명 만료, 상대의 연결 종료
  중 무엇이든) 그 `sessionId`는 폐기되어 재사용되지 않고, 다시 열려면 새 승인을 받는다.
  수명에는 상한을 둔다 — `sessionSeconds`(기본 300, 상한 900)와 유휴 제한 60초이며, 상한
  초과는 클램프하지 않고 승인 요청을 만들기 전에 거부한다. 서버 재시작 시 열린 세션은 모두
  닫힌다. `sessionId`는 추측할 수 없는 값이고, 연 주체가 아닌 호출자에게는 존재하지 않는
  것처럼 거부한다.

  각 호출은 보낼 바이트를 받고, 읽기에 바이트·시간 상한을 둔다(`readSeconds` 기본 5, 상한
  30). 이 도구는 네트워크 정책이 막아 둔 파드 포트에 도달할 수 있으므로, 승인 `context`에
  **대상 파드·포트, 세션 수명 상한, 그리고 첫 페이로드가 있으면 그 전문**이 들어가야
  한다(`prd-approval-gate` AC3). 세션을 연 뒤 보내는 페이로드는 승인 화면에 오르지 않는다 —
  **승인의 단위가 「이 페이로드」에서 「이 포트로의 이 수명 동안의 세션」으로 바뀐다는 것이
  이 AC가 받아들이는 대가다.** 대신 세션 안의 각 송신은 `sessionId`와 **세션을 연 승인 요청
  id**를 함께 감사 로그에 남겨, 어느 송신이 어느 승인 아래서 일어났는지 로그로 이어지게
  한다(`prd-approval-gate` AC8).
- **달성 가치**: V1, V3
- **검증 방법**: 픽스처 파드의 포트로 세션을 열고, 같은 `sessionId`로 두 번 이상 왕복해
  두 번째 왕복부터 승인 요청이 늘지 않음을 확인한다. 명시적 종료·유휴 만료·수명 만료 뒤
  같은 `sessionId`로의 호출이 거부되고 승인 요청도 만들지 않는다. `sessionSeconds`·
  `readSeconds` 상한 초과가 승인 요청 없이 거부된다. 다른 주체의 `sessionId` 사용이
  거부된다.
