...
판정 완료 합계 **<!-- docstring-합계 -->379<!-- /docstring-합계 -->줄**, 미판정 잔량
**<!-- docstring-잔량 -->843<!-- /docstring-잔량 -->줄**. 잔량은 이제 **전부**
`tests/integration/*_ac*.py` 60개 파일의 docstring이다 — `rct_20260905-0001` 슬라이스가 그중
**정형 문단만** 걷어냈고(아래), 나머지는 아직 판정받지 않았다. 공유 헬퍼 `_*.py` 7파일은
`rct_20260906-0001` 이 전수 판정해 위 원장 4행째로 등재했으므로 잔량에서 빠졌다.

> **`rct_20260905-0001` 슬라이스가 등재 없이 지운 것.** `tests/integration/*_ac*.py` 60개 파일의 모듈
