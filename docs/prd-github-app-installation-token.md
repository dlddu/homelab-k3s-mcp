...
- **발급하지 않는 권한**: `statuses`(commit status). 이 권한은 토큰으로 내주지 않고
...
### AC5: commit status 권한 배제
- **설명**: 이 도구가 발급하는 토큰에는 어떤 경로로도 `statuses` 권한이 실리지 않는다.
  1. `permissions`에 `statuses` 키가 있으면(`read`·`write` 무관) GitHub에 요청하기 전에 거부하고,
     문면에 `github_commit_status_create`를 쓰라고 안내한다.
  2. `permissions`를 생략하면 GitHub의 「생략 = 설치 권한 전부」에 기대지 않는다. 서버가 설치의
     권한 목록(`GET /app/installations/{id}`)을 읽어 `statuses`를 뺀 **명시적 맵**으로 요청한다.
     그 목록을 읽지 못하면 발급을 거부한다 — 생략 요청으로 폴백하지 않는다.
  3. GitHub가 돌려준 토큰의 `permissions`에 그래도 `statuses`가 있으면 그 토큰을
     폐기(`DELETE /installation/token`)하고 에러를 반환한다. 토큰 문자열은 응답에 싣지 않는다.
- **달성 가치**: V2, V3
- **검증 방법**: (1) `permissions={statuses: read}`·`{statuses: write}`·`{contents: read, statuses:
  write}` 호출이 모두 거부되고 상류 토큰 발급 요청이 0이다. (2) 설치 권한에 `statuses`가 있는
  상류에서 인자 없이 호출하면, 상류가 받은 발급 요청 본문에 `permissions`가 명시돼 있고 거기에
  `statuses`가 없으며, 응답의 `# Permissions:` 주석에도 없다. 설치 권한 조회가 실패하면 발급 요청
  없이 에러다. (3) 상류가 `statuses`가 실린 토큰을 돌려주면 폐기 요청이 한 번 가고, 도구 결과는
  에러이며 그 토큰 문자열을 포함하지 않는다.
- **호환성**: 인자 없는 기존 호출의 결과는 `statuses`가 빠지는 것 외에는 같다. 2026-09-19 현재 설치에는
  `statuses` 권한이 아예 없으므로 오늘 호출자가 보는 차이는 0이다 — 이 AC는 App에 그 권한을 추가하기
  **전에** 배포돼야 한다([배포 순서](prd-github-commit-status.md)).
