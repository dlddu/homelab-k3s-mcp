...
### 시나리오 14: 포트 포워드는 승인 하나로 세션 하나를 연다
- **사전 조건**: 동일, HTTP 를 서빙하는 파드
- **실행 단계**: `resource_port_forward`로 해당 포트에 첫 페이로드와 함께 세션 열기(승인) →
  반환된 `sessionId`로 두 번째·세 번째 왕복 → 명시적 종료 후 같은 `sessionId`로 재호출 →
  새 세션을 열고(승인) 유휴 제한을 넘긴 뒤 재호출 → `sessionSeconds`·`readSeconds` 상한을
  넘기는 값으로 호출 → 읽기 상한을 넘기는 응답을 내는 경로로 송신 → 세션을 연 주체가 아닌
  호출자로 `sessionId` 사용
- **기대 결과**: 세션을 열 때 승인 요청이 **정확히 하나** 생기고, 같은 세션의 두 번째 왕복부터
  승인 요청 수가 늘지 않음. 종료·유휴 만료 뒤의 `sessionId`는 거부되고 승인 요청도 만들지
  않음(세션은 한 번만 열린다). 수명·읽기 상한 초과는 승인 요청 없이 거부됨. 읽기 상한을 넘는
  응답은 잘리고 표시됨. 다른 주체의 `sessionId`는 존재하지 않는 세션처럼 거부됨. 승인 요청
  `context`에 **파드·포트·수명 상한과 첫 페이로드 전문**이 담기고, 세션 안의 각 송신이
  `sessionId`와 세션을 연 승인 요청 id와 함께 감사 로그에 남음
- **검증 AC**: AC14
