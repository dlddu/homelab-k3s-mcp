...
- **자동화**: Go 단위 `internal/mcp/attach_test.go::TestAttachJoinsTheRunningProcess`·
  ...
  **클러스터 호출 0·승인 요청 0** 거부, stdin 전달과 빈 문자열이 부재와 구분됨, 상한 잘림 표시,
  ...
  `tests/integration/resource_generic_ac13.py` 착지(2026-09-19 — 세 실행 단계를 실물 kubelet·실물
  gatekeeper 에서 왕복한다). **파드 로그로 「새 프로세스가 뜨지 않았다」를 확인하는 절과 stdin 에
  대한 대상 프로세스의 반응은 단위로 관측할 수 없어 그쪽이 진다**: 통합은 attach 가 돌려준 틱 줄이
  파드 로그에도 그대로 있음(=컨테이너 stdout, 곧 원래 살아 있던 프로세스의 스트림)과 틱 번호가
  1 에서 시작하지 않음을 함께 보고, `readSeconds=31` 이 **승인 요청을 만들기 전에** 거부되는지를
  호출 앞뒤 gatekeeper 요청 수로 재며, stdin 토큰의 되울림을 응답과 파드 로그 양쪽에서 찾는다.
  대상 파드(`stdin: true` 인 busybox 티커)는 이 시나리오 전용이라 그 파일이 직접 세운다
