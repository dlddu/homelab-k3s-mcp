...
- **건강 상태**: 🟡 **위험 있음** — 문서 계층의 연결은 모두 이어져 있으나,
  ...
  ⑵ `dear_baby_reset_user`가 게이트 밖에 있으며 ⑶ `k8s/rbac.yaml`에 죽은 부여 2건이 남아
  있다 (아래 "수용된 위험" 참조). 2026-09-14에 미결 하나가 닫혔다 — 두 PRD가 `rbac.yaml`을
  두고 어긋나던 것이 해소돼 쓰기 축의 문이 열렸다.
...
| 방어층 축소 | Secret 읽기 | 이전 설계는 도구 레이어 거부 + RBAC 에 `secrets` 규칙 부재의 **2중화**였다. 읽기를 허용하려면 RBAC 가 `secrets: get` 을 줘야 하므로, 이제 **도구 레이어의 게이트 판정이 유일한 경계**다. | ⚠️ **수용(2026-09-12)** — 거부하면 운영자가 `kubectl` 로 우회해 아무 기록도 남지 않는다. 전제 조건은 디스패처 단계 강제(AC1)와 fail-closed(AC5). **2026-09-14 정정**: 「RBAC 는 `get`·`list` 만 준다」는 AC16 이 Secret 쓰기를 게이트 대상으로 들인 뒤 낡은 문장이다 — 부여는 AC19 를 따르며 `secrets` 의 각 verb 는 그것을 행사하는 도구가 착지할 때 열린다. 오늘 `rbac.yaml` 에 `secrets` 규칙은 **없다** |
...
| RBAC 백스톱 소멸 | 전체 | `watch`·`deletecollection`·`attach`·`portforward`·`proxy`(`Node` 포함)·`secrets` 쓰기가 차례로 도구가 되면서 **금지 목록이 비었다**. RBAC 가 도구 표와 정확히 같아진다는 것은 곧 **RBAC 가 더는 백스톱이 아니라는 뜻**이다 — 게이트에 결함이 생기면 apiserver 가 막아 줄 것이 없다. | ⚠️ **수용(2026-09-12)** — 완화는 AC1(디스패처 강제)·AC5(fail-closed)·AC11(게이트 권한 선언)이며, 이 셋이 이제 구조 전체를 지탱한다. **2026-09-14 보강**: 「테스트 밀도를 높게 잡을 것」은 지킬 시점이 없는 다짐이었으므로, AC19 에 선행 조건으로 박았다 — **부여를 넓히는 PR 은 그 verb 가 게이트를 타는 경로를 라이브로 관측하는 e2e 를 같은 PR 에 담아야 한다.** 백스톱은 한 번에 사라지지 않고 쌍 단위로 사라지므로, 완화도 그 쌍이 열리는 자리에 둔다 |
...
| ~~두 PRD 가 `k8s/rbac.yaml` 을 두고 어긋난다~~ | `prd-platform-auth-safety` AC3 ↔ `prd-resource-generic` AC19 | AC3 이 부여 집합을 축자로 못박고 「시크릿 읽기 권한은 부여하지 않는다」로 닫는 반면 AC19 는 같은 파일이 도구 표 ∪ 게이트 선언과 같아야 한다고 적어, 둘을 동시에 만족하는 `rbac.yaml` 이 없었다. 2026-09-13 읽기 축 슬라이스가 AC19 쪽으로 파일을 넓혔다가 `platform_auth_safety_ac3.py` 의 등가 단언에 걸려 되돌렸다. | ✅ **해소(2026-09-14)** — **AC19 가 이긴다**: 파일의 내용은 AC19 가 소유하고(등록된 도구 ∪ 게이트 선언), AC3 은 열거를 그만두고 **실효 권한 = 바인딩된 역할**이라는, 매니페스트를 읽어서는 볼 수 없는 한 걸음만 책임진다. 근거는 AC3 의 목록이 독립 정책이 아니라 **그 시점 도구들이 필요로 하던 것의 전사**였다는 것이다 — 죽은 부여 둘(`watch`·`pods/exec get`)이 그 자국이고, 워크로드에 `patch`(쓰기)를 주면서 `delete`/`create` 만 금지하는 선은 원리가 아니다. 기계 쪽 전사도 함께 걷어 `platform_auth_safety_ac3.py` 가 기대 답을 **런타임에 역할에서 읽는다**. 권한 변경 0 |
| **죽은 부여 2건** | `k8s/rbac.yaml` — 워크로드 `watch`, `pods/exec` `get` | AC19 는 아무도 행사하지 않는 쌍을 실패로 본다. 실측: 비-테스트 코드에 `Watch(` 호출이 **0건**이고 `resource_watch` 는 미구현이다. `pods/exec` 의 `get` 은 WebSocket executor 폴백을 위해 남겼다고 적혀 있으나 `internal/k8s/client.go` 가 `remotecommand.NewSPDYExecutor(..., "POST", ...)` 로 고정돼 있어 행사되는 것은 `create` 뿐이다. 둘 다 이전 판 AC3 의 축자 부여 목록에 올라 있어서 살아남았다. | ⬜ **제거 대기(2026-09-14)** — 전사를 걷어낸 PR 에 접어 넣지 않았다. 권한을 줄이는 변경은 **자기 PR 로** 가야 되돌릴 때 그것만 되돌아간다. AC19 정적 대조가 서면 그 검사의 첫 두 발화가 이것이다 |
...
> **2026-09-14 — 3번을 막고 있던 문 하나를 열었다. 코드도 권한도 그대로다.**
> 위 두 슬라이스가 남긴 「두 PRD가 `k8s/rbac.yaml`을 두고 어긋난다」를 결정으로 닫았다:
> **파일의 내용은 `prd-resource-generic` AC19가 소유하고**, `prd-platform-auth-safety` AC3은
> 열거를 그만두고 **실효 권한 = 바인딩된 역할**만 책임진다. 두 문서가 경쟁하는 것이 아니라
> 한쪽이 낡았다고 본 근거는 AC3의 목록이 `resource_*` 이전 도구 집합이 필요로 하던 것의
> **전사**라는 점이다 — 그 목록이 요구하는 `watch`는 코드에 호출이 0건이고,
> `pods/exec`의 `get`은 존재하지 않는 WebSocket 폴백을 위해 남아 있으며, 워크로드에
> `patch`(쓰기)를 주면서 `delete`/`create`만 금지하는 선은 원리가 아니라 그때의 자국이다.
>
> **결정보다 중요한 것은 전사를 기계에서 걷어낸 것이다.** `platform_auth_safety_ac3.py`가
> AC 문언을 옮겨 적은 집합을 **등가로** 강제하고 있었고, 그래서 문서를 고쳐도 파일은 고칠 수
> 없었다(읽기 축 슬라이스가 실제로 여기 걸려 되돌렸다). 이제 기대 답을 **런타임에 바인딩된
> 역할에서 읽고**, 위험 쌍 카탈로그를 그 부여로 갈라 허용/거부를 단정한다 — 부여가 넓어지면
> 그 쌍이 자동으로 거부 쪽에서 허용 쪽으로 옮겨 간다. 부여됐는데 카탈로그에 없는 쌍은
> 실패시킨다(묻지 않은 권한은 통과하므로). 옛 판이 단정하던 32쌍은 전부 같은 판정으로 남고,
> 묻는 쌍은 32 → 55로 늘었다.
>
> **함께 고친 낡은 문언 넷**(전부 같은 파일, 전부 구현·다른 AC와 이미 어긋나 있던 것):
> ⑴ `prd-resource-generic` AC17의 「`rbac.yaml`이 `secrets`에 쓰기 verb를 주지 않으므로
> 권한 밖」 — 같은 문서 AC16·AC19와 정면으로 모순하는 이전 판의 잔해. ⑵ 도구 개요 절이
> RBAC 대조를 「AC20」이라 가리키던 것 → AC19. ⑶ **AC2의 `Accept` 문언이 아직 `v=1`이었다**
> — 바로 위 슬라이스를 멈춰 세운 그 값이고, 구현만 고쳐지고 문서는 그대로였다.
> ⑷ **`prd-approval-gate` AC11의 PartialObjectMetadata `Accept`도 같은 `v=1`이다.**
> 이쪽은 미구현이라 지뢰가 살아 있었다 — 문언대로 구현하면 apiserver가 오류 대신 전체 객체로
> 답하므로 **게이트가 승인 전에 Secret 값을 읽게 된다.** AC11이 존재하는 이유가 그 자리에서
> 뒤집힌다. ⑶⑷ 둘 다 「손으로 적지 말고 `metav1`에서 파생하라」를 문언에 박았다.
>
> **범위 밖으로 남긴 것**: `k8s/rbac.yaml`의 규칙 줄은 **한 글자도 바꾸지 않았다**(주석만
> 갱신했고, 주석·빈 줄을 지운 대조가 바이트 동일이다). 죽은 부여 2건 제거·AC19 정적 대조
> 구현·쓰기 축 도구는 각자의 PR로 간다. 권한을 줄이는 변경조차 이 PR에 접지 않은 것은,
> 되돌릴 때 그것만 되돌아가게 하기 위해서다.
...
3. `k8s/rbac.yaml`을 재검토한다. `resource_*`는 종류를 입력으로 받으므로 현재의
   워크로드 한정 규칙으로는 대부분의 종류가 AC18의 "권한 밖" 에러로 떨어진다.
   무엇을 열지는 이 문서가 정하지 않는다 — 그 파일의 단일 소유자는
   **`prd-resource-generic` AC19**이고(2026-09-14 결정), 그 파일을 고치는 PR이 근거를
   함께 적는다. **형태도 AC19가 못박는다**: 쌍 집합이 **그 빌드에 등록된 도구**가 선언한
   것 ∪ 게이트 선언(`prd-approval-gate` AC11)과 정확히 같다. 도착점의 표가 아니라
   등록된 것이 좌변이므로 **미리 열어 두는 부여는 그 자체로 위반**이다.
   **순서 제약**: 각 쌍은 그것을 행사하는 도구가 착지하는 PR에서, 그 verb가 게이트를 타는
   경로를 관측하는 e2e와 **함께** 열린다. 게이트 구현이 먼저, 부여가 나중이라는 제약은
   이 규칙 안에 흡수됐다 — 도구 없이 열리는 부여가 없으므로 순서가 뒤집힐 자리가 없다.
   반대로 **죽은 부여 제거(`watch`·`pods/exec` `get`)는 지금 해도 안전하다**(위
   「죽은 부여 2건」).
...
| **test-approval-gate.md#시나리오 1** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 2** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 3** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 4** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 5** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 6** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 7** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 8** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-approval-gate.md#시나리오 9** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
...
| **test-resource-generic.md#시나리오 8** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 9** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 10** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 11** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 12** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 13** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 14** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 15** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 16** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 17** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 18** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 19** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
| **test-resource-generic.md#시나리오 20** | `resource_*` 도구군이 **아직 구현되지 않았다** — 승인 게이트의 판정 코어(`internal/gatekeeper`)와 디스패처 강제는 착지했으나, `internal/mcp` 에 게이트를 타는 도구가 하나도 없어 e2e 가 관측할 대상 자체가 없다. | 이 렌즈 | ⑴ `prd-resource-generic` · `prd-approval-gate` 의 도구 계층이 구현되고, ⑵ kind 에 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`)가 서면 해제된다 |
...
|------|-----------|-----------|-----------|
| 2026-09-14 | **`k8s/rbac.yaml` 의 소유 문서를 하나로 정리했다 — 권한 변경 0** — 두 PRD 가 같은 파일의 내용을 각자 적고 있어 어느 쪽을 고쳐도 다른 쪽이 거짓이 됐고, 그 교착이 `resource_*` 쓰기 축 전체를 막고 있었다. **AC19 가 이긴다**: 파일의 내용은 `prd-resource-generic` AC19 가 소유하고(좌변은 도착점의 도구 표가 아니라 **그 빌드에 등록된 도구** ∪ 게이트 선언 — 미리 열어 두는 부여는 그 자체로 위반이다), `prd-platform-auth-safety` AC3 은 축자 열거를 걷고 **실효 권한 = 바인딩된 역할**이라는, 매니페스트를 읽어서는 원리상 볼 수 없는 한 걸음만 책임진다(다른 바인딩·그룹 부여는 파일에 흔적을 남기지 않는다). AC3 을 낡은 쪽으로 판정한 근거는 **그 목록이 정책이 아니라 전사**라는 것이다 — 요구하는 `watch` 는 비-테스트 코드에 호출이 **0건**이고, `pods/exec` 의 `get` 은 `client.go` 가 `NewSPDYExecutor(..., "POST", ...)` 로 고정돼 존재하지 않는 폴백을 위해 남아 있으며, 워크로드에 `patch`(쓰기)를 주면서 `delete`/`create` 만 금지하는 선은 원리가 아니다. **결정보다 무거운 것은 기계에서 전사를 걷어낸 것**이다 — `platform_auth_safety_ac3.py` 가 AC 문언 사본을 **등가로** 강제하던 것을 **런타임에 역할에서 읽는** 방식으로 바꿨다(옛 판이 단정하던 32쌍은 전부 같은 판정, 묻는 쌍은 32 → 55, 부여가 넓어지면 그 쌍이 자동으로 거부 → 허용으로 이동, 부여됐는데 카탈로그에 없으면 실패). 백스톱 소멸의 완화도 다짐에서 **선행 조건**으로 옮겼다(넓히는 PR 은 그 verb 의 게이트 경로 e2e 를 동반한다). 함께 낡은 문언 넷을 고쳤다 — AC17 의 「secrets 쓰기는 권한 밖」(같은 문서 AC16·AC19 와 모순), 「AC20 의 RBAC 대조」(→ AC19), **AC2 의 `Accept` `v=1`**(직전 슬라이스를 멈춰 세운 값인데 구현만 고쳐졌다), **`prd-approval-gate` AC11 의 PartialObjectMetadata `v=1`**(미구현이라 지뢰가 살아 있었다 — 문언대로 구현하면 게이트가 **승인 전에 Secret 값을 읽는다**). `k8s/rbac.yaml` 의 규칙 줄은 무변경(주석만 갱신, 주석·빈 줄 제거 후 바이트 동일). 근거는 `rct_20260914-0002`. | `rbac.yaml` 소유자 2(모순) · AC3 = 축자 전사 8쌍 · e2e 기대값 = AC 사본 · 묻는 쌍 32 · 문서 내 `;v=1` 2곳 · 쓰기 축 착수 불가 | `rbac.yaml` 소유자 1(AC19) · AC3 = 실효 권한 불변식 · e2e 기대값 = 런타임 역할 · 묻는 쌍 55 · `;v=1` 0곳 · 쓰기 축 문 열림(부여는 도구와 함께) |
