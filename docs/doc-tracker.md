...
- 등록 도구: **19종** (`internal/mcp/toolslist.go` = `tests/integration/_helpers.py::EXPECTED_TOOLS`) — 그중 게이트 대상 쓰기 도구는 `resource_create`·`resource_update`·`resource_patch`·`resource_delete` 4종이고, 게이트 밖 예외는 **1종**(`dear_baby_reset_user`)이다. `resource_watch` 는 쓰기가 아니므로 이 둘 어디에도 들어가지 않는다 — 민감 종류(`RESOURCE_GATED_KINDS`)를 대상으로 할 때만 `resource_get` 과 같은 조건으로 게이트를 탄다
...
| **test-resource-generic.md#시나리오 11** | `resource_exec` 은 `internal/mcp/toolslist.go` 에 등록돼 있지 않다. 다중 컨테이너 선행은 이미 갖춰져 있다: `tests/k8s/kind/resource-generic-fixture.yaml` 의 `workload-test/resource-generic-multi` Deployment 가 `chatty`(busybox)·`quiet`(pause)를 정의하고, `.github/workflows/ci.yml` 이 이 매니페스트를 적용하며 `tests/integration/resource_generic_ac5.py` 가 이 워크로드를 대상으로 컨테이너 구성을 단언한다. 이는 픽스처의 존재·CI 배선 근거이며 시나리오 11의 exec·출력 상한 검증 완료를 뜻하지 않는다. 승인을 태울 `tests/k8s/kind/gatekeeper-fixture.yaml` 은 아직 없다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ `resource_exec` 등록, ⑵ 실물 gatekeeper 픽스처 착지. 두 조건이 충족되면 기존 다중 컨테이너 픽스처를 이용해 `resource_generic_ac11.py` 의 시나리오 전체를 저작·실행한다. **RBAC 는 이미 `pods/exec` 을 허용한다**(`k8s/rbac.yaml` 의 `cluster-admin` 바인딩) |
...
| **test-resource-generic.md#시나리오 17** | (a) 의 `kind=Secret` `resource_list` 와 (c')·(d) 가 전제하던 `nodes/proxy`·`secrets` 의 `watch` 는 2026-09-14 RBAC 개정으로 전부 권한 안에 들어왔다. 남은 것은 (c) 다 — 스트림 넷(`exec`·`attach`·`port_forward`·`proxy`)이 미등록이고(레포 전체 grep 0 히트), (d) 의 승인 갈래는 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없어 태울 수 없다. (b) 의 `RESOURCE_GATED_KINDS` 확장 경로는 구현돼 있다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 스트림 도구 넷 등록 **그리고** ⑵ 실물 gatekeeper 픽스처. 난수 토큰 Secret 과 그 토큰을 서빙하는 HTTP 파드 픽스처는 이 렌즈 자신의 저작이라 차단 요인이 아니다 |
...
|------|-----------|-----------|-----------|
| 2026-09-17 | `rct_20260915-0008` 시도 3 — rg#9 apply 케이스의 identity 주입 + 진단장치(PR #122). approvedPatch의 apply 분기가 applied config '객체' 해석에 필요한 좌표 `apiVersion`·`kind`를 호출자가 쓰지 않았을 때만 채우고 호출자 불일치를 제출 전에 거부하며, 400/422 버킷은 접두어·종결을 보존한 채 apiserver `Status.message`를 괄호로 첨부한다. 병합 트리(PR #106 픽스처 + #122)의 CI integration에서 apply 케이스 ok(ok 라인 9/9 · 배차 43/3)를 실측해 PRD AC9 apply 절과 위험 행을 실측에 수렴시켰다. PR은 draft 유지 — 머지 판정은 검토의 몫이다. | apply 본문이 gvk 없이 나가 merge·strategic·json은 통과하나 apply만 400/422로 결정적으로 거부(3회 시도 원인 미판명) | apply 본문에 좌표 gvk 주입으로 실측 통과; 거부 시 apiserver detail이 문구에 남는다(원장 미판정 잔량 9줄) |
