...
- 등록 도구: **16종** (`internal/mcp/toolslist.go` = `tests/integration/_helpers.py::EXPECTED_TOOLS`)
...
### 시나리오 레지스트리 (78) — ✅ 전용 파일 47 · ⬜ 분할 대기 0 · ⏳ 구현 대기 30 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **78 − 1(예외) − 30(구현 대기) = 47 = 매칭 파일 47**, 공백 **0**.
...
| test-resource-generic.md#시나리오 20 | 종류 해석 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 미작성) |

### ⏳ 구현 대기 (30) — 규칙 6 등재 (1:1 계수에서 제외)
...
| **test-resource-generic.md#시나리오 9** | `resource_patch` 가 등록돼 있지 않다(레포 전체 grep 0 히트). `patch` 는 부여돼 있으나(2026-09-14 개정 이후 `cluster-admin`) 네 `patchType` 을 태울 도구가 없고, 각 호출이 승인을 요구하는데 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ `resource_patch` 등록(네 `patchType` 지원) **그리고** ⑵ 실물 gatekeeper 픽스처. RBAC 는 `patch` 를 이미 준다 — 이 행은 rbac 충돌에 걸려 있지 않다 |
...
| **test-resource-generic.md#시나리오 16** | **읽기 절반은 섰다** — `kind=Secret` 의 `get` 은 민감 종류로 판정돼 게이트를 탄다(`internal/mcp/gate.go:223-243`). 그러나 시나리오는 `get`·`watch`·`create`·`update`·`patch`·`delete` **여섯 verb 전부**와 승인 후 값 확인을 요구하는데 나머지 다섯 도구가 미등록이고(레포 전체 grep 0 히트), `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. `secrets` 자체는 2026-09-14 RBAC 개정으로 권한 안이다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 나머지 다섯 도구 등록 **그리고** ⑵ 실물 gatekeeper 픽스처. 난수 토큰 Secret 픽스처는 이 렌즈 자신의 저작이라 차단 요인이 아니다 |
...
| **test-resource-generic.md#시나리오 20** | **계획 시점의 두 근거가 둘 다 죽었다.** ⑴ 「오타 호출에 **후보 제시**」는 이 슬라이스(`rct_20260914-0004`)가 후보 산출을 **편집 거리**(길이에 비례하는 예산 + 기존 부분 문자열 매칭 유지, 가까운 순 정렬 뒤 상위 5개)로 바꿔 선다 — 시나리오가 지목한 오타 `Deploymnt` 에 `apps/v1/Deployment` 가 단독 후보로 나오는 것을 실측 카탈로그(클러스터 147종)와 Go 단위 `resource_test.go::TestUnknownKindSuggestsCandidates` 로 확인했다. ⑵ 「CRD 종류의 `list` 가 `k8s/rbac.yaml` 의 ClusterRole 밖」은 **#83 이 RBAC 를 경계에서 내리며 사라졌다** — 그 파일은 이제 `cluster-admin` 바인딩 하나이고 종류를 좁히는 ClusterRole 자체가 없다. **남은 차단은 전용 e2e 파일이 아직 없다는 것 하나뿐**이고, 그것이 규칙 4 가 「아직 자동화하지 못했다」를 예외가 아니라 이 표로 보내는 자리다. `api_resources` 자체와 재기동 없는 캐시 무효화(`internal/k8s/resource.go`)는 서 있다. | docs-impl(전용 e2e 저작) → 이 렌즈(e2e) | 전용 파일 `tests/integration/resource_generic_ac20.py` 가 저작되면 해제된다 — 그 시점에 구현 대기 −1 · 1:1 대상 +1 · 매칭 파일 +1 로 집계가 함께 움직인다. **gatekeeper 픽스처는 무관하다** |
