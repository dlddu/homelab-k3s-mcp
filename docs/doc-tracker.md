...
| 2026-09-18 | **`rct_20260918-0003` — 「픽스처가 유일한 잔여였던」 구현 대기 6행을 전용 e2e 로 닫았다** — PR #106 이 실물 gatekeeper 픽스처를 착지시키면서 구현 대기 표 14행에 `⟨2026-09-18 갱신⟩` 주석으로 「픽스처 부재는 더는 차단 요인이 아니다」를 스스로 적었는데, 그중 7행은 「위에 적힌 나머지 조건」이 비어 있어 잔여가 *전용 e2e 파일 저작* 하나였다. **규칙 4 가 저작 대기를 등재 사유에서 배제**하므로 그 행들은 등재 근거를 잃은 상태였다(게이트는 행 **수**만 세므로 이 어긋남을 원리적으로 보지 못하고 계속 `공백 0` 을 냈다). 그 7 중 **6 을 저작해 닫고**, 나머지 1(`test-resource-generic.md#시나리오 18`)은 **행의 근거를 다시 썼다** — 그 시나리오의 사전 조건이 「RBAC 에 없는 종류」인데 `k8s/rbac.yaml` 이 2026-09-14 이래 `cluster-admin` 바인딩이고 모든 변형 배포가 같은 SA 를 쓰므로 403 을 받을 좌표가 하나도 없다. 즉 그 행의 실제 차단은 저작이 아니라 **하네스가 도달할 수 없는 사전 조건**(규칙 6)이고, 해제 조건을 「좁은 ClusterRole 을 가진 변형 배포」로 바꿔 적었다. 신설 파일 6: `approval_gate_ac6.py`(승인↔실행 상태 동일성 — patch·scale·Secret get·exec 는 재확인 거부, create 는 재확인이 아니라 apiserver 409) · `approval_gate_ac10.py`(context·서버 로그·에러·list 네 표면에서 토큰 부재, 승인된 get 응답만이 대조군) · `resource_generic_ac6.py`(창 안 MODIFIED·61 거부·resume·민감 종류 승인) · `resource_generic_ac7.py`(409·문서별 승인·거절 시 무생성·부분 실패 무롤백) · `resource_generic_ac11.py`(stdout·stderr 분리·후보 이름·256KiB 잘림 표시) · `resource_generic_ac16.py`(여섯 verb 미승인 거부·create context 가림·list 예외). **제품 코드 0줄** — `internal/`·`cmd/`·`k8s/` 무접촉이고 산출물은 e2e 와 등재 문서뿐이다(이 렌즈의 경계 조항). 규칙 7 에 따라 `test-approval-gate.md`(시나리오 6·10)·`test-resource-generic.md`(시나리오 6·7·11·16)의 `자동화` 필드도 같은 PR 에서 갱신했다. **집계**: `check_ac_mapping.py` 일곱 값 `85·1·26·58·58·0·0` → `85·1·20·64·64·0·0`. | 구현 대기 26행 — 그중 7행은 잔여가 저작뿐이라 규칙 4·6 아래 등재 근거가 없었고, 시나리오 18 행은 이미 무너진 「승인 쪽 차단」을 근거로 들고 있었다 | 구현 대기 **20행**(전부 도구 미등록·하네스 선행 등 규칙 6 정당) · 전용 파일 **64** · 공백 0. 남은 잔여의 소관은 도구 12행이 `tbm_homelab-k3s-mcp-docs-impl`, ag#5 가 `tbm_homelab-k3s-mcp-e2e-mock-policy`, 시나리오 18 과 session 계열이 이 렌즈 |
