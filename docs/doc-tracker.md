...
- 등록 도구: **18종** (`internal/mcp/toolslist.go` = `tests/integration/_helpers.py::EXPECTED_TOOLS`) — 그중 게이트 대상 쓰기 도구는 `resource_update`·`resource_patch`·`resource_delete` 3종이고, 게이트 밖 예외는 **1종**(`dear_baby_reset_user`)이다. `resource_watch` 는 쓰기가 아니므로 이 둘 어디에도 들어가지 않는다 — 민감 종류(`RESOURCE_GATED_KINDS`)를 대상으로 할 때만 `resource_get` 과 같은 조건으로 게이트를 탄다
...
| **AC6 서버사이드 프리컨디션 — update 완료, patch 잔여** | `resource_update`·`resource_patch` | `resource_update`는 게이트가 승인 화면 작성 시 읽은 `resourceVersion`을 요청별 상태로 전달하고, 전체 객체 PUT과 `scale` PUT의 `metadata.resourceVersion`에 넣는다. 호출자 매니페스트의 버전이 다르면 쓰기 전에 거부한다. 갱신 시 `resourceVersion` 충돌은 자동 거부로 현재 호출을 종료한다. 재확인 직후 apiserver가 409를 반환해도 추가 쓰기·버전 갱신·새 승인 요청을 자동으로 만들지 않는다. 호출자가 나중에 별도로 시도하려면 새 승인이 필요하다. 게이트 재확인과 민감 종류의 metadata-only 읽기는 유지한다. | **부분 완료(2026-09-15, `rct_20260915-0011`)** — update 두 경로는 승인 버전 누락 시 API 요청 전에 거부하고, 본문은 복사해 보낸다. 디스패처→서비스 전달 및 HTTP 경계 회귀 테스트로 승인 버전·1회 PUT·충돌 거부를 검증한다. 시도 2는 전체 객체·scale의 재확인/서비스 충돌에서 에러 응답·승인 1회·소비됨·추가 읽기/재승인/재시도 없음을 명시적으로 단언한다. 실물 gatekeeper E2E는 자매 모델/PR #106 소관이며 이 변경은 시나리오 완료로 세지 않는다. **잔여**: `resource_patch`의 `merge`·`strategic`·`json`·`apply`별 원자적 조건과 충돌 계약, 다른 verb의 잔여 정합성 및 미등록 6종. AC6 전체 완료를 뜻하지 않는다. |
...
| test-resource-generic.md#시나리오 7 | 생성은 덮어쓰지 않는다 | ⏳ 구현 대기 (규칙 6 — 아래 행별 근거) |
...
| test-resource-generic.md#시나리오 16 | 민감 종류는 읽기도 쓰기도 승인을 거친다 | ⏳ 구현 대기 (규칙 6 — 아래 행별 근거) |
...
| **test-approval-gate.md#시나리오 3** | 「게이트 대상 verb 각각」 아홉 중 `update`·`patch`·`delete`는 등록돼 있다(`internal/mcp/toolslist.go`·`gate.go`·`tests/integration/_helpers.py`). 스케일의 현재→목표 레플리카와 재시작 어노테이션 패치도 기존 경로다. 남은 여섯은 `resource_create`·`resource_delete_collection`·`resource_exec`·`resource_attach`·`resource_port_forward`·`resource_proxy`다. 생성된 `context`를 수집할 실물 gatekeeper 픽스처도 main에는 없다. 세 verb만 단언해서 「각각」을 충족했다고 세지 않는다. | docs-impl(남은 도구) → 이 렌즈(e2e; 공통 픽스처는 PR #106) | ⑴ 남은 여섯 도구와 각 verb의 시나리오상 context 계약 구현, ⑵ 실물 gatekeeper 픽스처 착지. 그 뒤 아홉 verb의 상세·좌표 해석 실패까지 전용 `approval_gate_ac3.py`에서 검증 |
...
| **test-approval-gate.md#시나리오 6** | `resource_patch`·`resource_update(subresource=scale)`·민감 종류 `resource_get`은 등록돼 있고, 게이트는 승인 전 읽은 `resourceVersion`·`uid`를 실행 직전에 재확인한다(`internal/mcp/gate.go::confirmTargetUnchanged`). 업데이트 미등록 주장은 더는 맞지 않는다. 현재 시나리오의 나머지 두 경로인 `resource_exec`(파드 재생성 뒤 uid 불일치)와 `resource_create`(승인 중 같은 이름 생성 뒤 409)는 미등록이다. 승인 요청과 판정 사이에 외부 변경을 넣을 실물 gatekeeper 픽스처도 없다. 기존 재확인은 읽기와 쓰기 사이의 원자적 프리컨디션 완료를 뜻하지 않는다. | docs-impl(create·exec 및 제품 잔여) → 이 렌즈(e2e; 공통 픽스처는 PR #106) | ⑴ `resource_create`·`resource_exec`의 해당 거부 계약 구현, ⑵ 실물 gatekeeper 픽스처 착지. 이후 `approval_gate_ac6.py`에서 patch·scale·Secret get·exec·create 다섯 케이스를 모두 검증하며, create는 재확인 대신 API의 409와 승인 context의 resourceVersion 부재를 확인 |
...
| **test-approval-gate.md#시나리오 10** | 훑어야 할 셋 중 **둘이 섰다** — `resource_get` 과 `resource_patch` 가 등록됐고, 쓰기 경로의 `context` 마스킹(키 이름 + 바이트 수)도 구현돼 있다(`gate.go` 의 `writesASensitiveKind`→`maskCredentialValues`). RBAC 도 `cluster-admin` 이라 승인 후 정상 응답에는 값이 오므로 「오직 그때만 등장한다」를 단정할 수 있다. 남은 것은 `resource_create` 미등록(0히트)과 실물 gatekeeper 픽스처 부재다. 난수 토큰 Secret 픽스처는 이 렌즈 자신의 저작이라 차단 요인이 아니다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ `resource_create` 등록 **그리고** ⑵ 실물 gatekeeper 픽스처. **직전 판의 `secrets` 부여 조건은 죽었다**(`rbac.yaml` = `cluster-admin`) |
...
| **test-resource-generic.md#시나리오 7** | `resource_create`는 아직 등록되지 않았다. 현재 시나리오는 단건 생성·중복 이름 409·두 문서의 개별 승인뿐 아니라 **둘째 승인 거절 시 아무것도 생성하지 않음**과 **둘 다 승인 뒤 둘째 생성이 409면 첫 객체를 남기고 결과를 보고함**도 요구한다. 따라서 예전 「세 단계」의 성공 경로만으로는 닫을 수 없다. 실물 gatekeeper 픽스처도 main에 없다. RBAC는 cluster-admin이므로 과거 create 권한 충돌을 선행으로 되살리지 않는다. | docs-impl(create의 전체 계약) → 이 렌즈(e2e; 공통 픽스처는 PR #106) | ⑴ 전 문서 승인 뒤 쓰기를 시작하고 실행 중 실패는 성공·실패·미시도 좌표로 보고하는 `resource_create` 구현, ⑵ 실물 gatekeeper 픽스처 착지. 그 뒤 `resource_generic_ac7.py`에서 단건·중복·다문서 성공·승인 거절·부분 실행 실패를 전부 검증 |
...
| **test-resource-generic.md#시나리오 16** | 시나리오의 여섯 verb 중 `get`·`watch`·`update`·`patch`·`delete` 다섯은 등록돼 있다(`toolslist.go`·`gate.go`·`EXPECTED_TOOLS`). 민감 종류의 get/watch와 모든 쓰기는 게이트를 타고, 기존 쓰기 context 값 가림도 구현돼 있다. **남은 도구는 `resource_create` 하나**다. 시나리오는 바로 create의 승인 context에서 난수 토큰이 가려지고 승인 후 실제 Secret 값이 생기는 것을 요구하므로 기존 patch 단위 테스트로 대체할 수 없다. 실물 gatekeeper 픽스처와 여섯 verb를 모두 태우는 전용 통합 파일도 없다. | docs-impl(create) → 이 렌즈(e2e; 공통 픽스처는 PR #106) | ⑴ 민감 값 가림·승인 후 생성 계약을 갖춘 `resource_create` 구현, ⑵ 실물 gatekeeper 픽스처 착지. 그 뒤 `resource_generic_ac16.py`에서 여섯 verb의 미승인 거부, create context·생성 값, list와 ConfigMap get 대조군을 함께 검증. 난수 토큰 Secret 픽스처는 이 렌즈 자신의 저작 |
