...
### 시나리오 레지스트리 (78) — ✅ 전용 파일 49 · ⬜ 분할 대기 0 · ⏳ 구현 대기 28 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **78 − 1(예외) − 28(구현 대기) = 49 = 매칭 파일 49**, 공백 **0**.
...
| test-resource-generic.md#시나리오 5 | 서브리소스 조회 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 미작성) |
...
| **test-resource-generic.md#시나리오 5** | 🟢 **2026-09-14 — 이 행이 유일하게 들고 있던 차단 요인이 고쳐졌다.** 직전 판이 배포된 제품으로 재실측해 반증한 것(「`container` 누락이 거부되고 **후보 이름이 제시됨**」이 서지 않는다 — 응답이 `kubernetes api error: the server rejected our request for an unknown reason (get pods ⟨이름⟩)` 이고 후보가 없다)을 `rct_20260914-0015` 가 **`podLogs` 에서 고쳤다**. 원인은 의도가 아니라 사고였다 — client-go 의 `DoRaw` 는 본문을 에러와 **함께** 돌려주면서도 응답이 텍스트일 때만 그 본문을 메시지로 읽고, apiserver 는 `pods/log` 를 JSON `Status` 로 거절한다. 그래서 apiserver 가 실어 보낸 「a container name must be specified for pod ⟨이름⟩, choose one of: [...]」 가 **이미 손에 있는 채로** 버려지고 있었다. 이제 `Status.message` 를 살려 답한다(403 은 이 서버 자신의 grant 문면을 그대로 유지한다 — 대조군 단위 테스트가 그 경계를 고정한다). 로그 축 나머지 넷과 `subresource=scale` 은 직전 실측대로 이미 서 있다. | 이 렌즈(e2e 저작) | 전용 e2e `resource_generic_ac5.py` 저작(폐기되는 `workload_logs_ac{1,2,3,4}.py` 의 단언 승계). **docs-impl 쪽 차단 요인은 0이고 RBAC·gatekeeper 픽스처는 무관하다** — 시나리오 3 행과 같은 이유로 이 행도 정의상 **규칙 1 의 공백**이며, 저작이 끝나면 표에서 빠진다 |
