...
| **test-resource-generic.md#시나리오 3** | 한 겹만 남았다. ⑴ 「사전 조건인 **ConfigMap 120개** 를 읽을 `configmaps list` 가 권한 밖」은 2026-09-14 RBAC 개정으로 죽었다(`cluster-admin`). 120개 픽스처 저작은 이 렌즈 자신의 트리 안이라 차단 요인이 아니다. 남은 ⑵ — 기대 결과의 「`limit=1000` 은 500 으로 **강제 하향**」이 구현과 어긋난다 — `internal/mcp/resource.go:67` 이 상한 초과를 클램프하지 않고 **거부**하고 `internal/k8s/resource.go:41-46` 주석이 그 선택을 의도로 명시한다. 기본 100 과 `continue` 토큰은 서 있다. | 제품 문서/docs-impl(상한 처리) → 이 렌즈(e2e) | 상한 초과 처리가 시나리오 문면과 일치하면 해제된다. 구현을 하향으로 고치거나 시나리오를 거부로 고치거나인데 **어느 쪽인지는 이 렌즈가 정하지 않는다** — 시나리오가 이 모델의 SSOT 이므로 문면 변경은 제품 문서 체계의 판단이다 |
| **test-resource-generic.md#시나리오 5** | 🔴 **2026-09-14 에 배포된 제품으로 재실측해 이 행의 직전 근거(「남은 것은 저작뿐」)를 취소한다.** 로그 축 다섯 중 넷과 `subresource=scale` 은 실제로 서 있다(라이브 확인: `deployments/scale` 이 `autoscaling/v1` Scale 을 돌려준다). 그러나 다섯째 「`container` 누락이 거부되고 **후보 이름이 제시됨**」이 서지 않는다 — 컨테이너 둘인 파드를 `container` 없이 읽으면 응답이 `kubernetes api error: the server rejected our request for an unknown reason (get pods ⟨이름⟩)` 이고 **후보 이름이 없다**. apiserver 가 400 본문에 담아 보내는 「choose one of: [...]」 가 client-go 의 일반 메시지로 덮여 `internal/k8s/resource.go` 의 `podLogs` 에 도달하지 못한다. 같은 파드를 `container` 를 주고 읽으면 성공하므로 거부 자체는 컨테이너 모호성 때문이 맞다. 이것은 시나리오 20 ⑴ 과 같은 계열의 **문서↔구현 불일치**이지 하네스 미비가 아니다. | docs-impl(후보 산출) → 이 렌즈(e2e) | `container` 누락 거부가 후보 컨테이너 이름을 싣도록 정정되면 해제된다(파드 스펙에서 이름을 읽어 메시지에 붙이거나, apiserver 본문을 보존하거나). **차단 요인은 그 하나뿐이다** — RBAC·gatekeeper 픽스처는 무관하고, 나머지 네 축과 scale 은 이미 관측 가능하다 |
