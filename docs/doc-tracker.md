...
| **test-resource-generic.md#시나리오 11** | **⑴ 도구 축은 2026-09-17 에 닫혔고 ⑵ 픽스처 축은 2026-09-18(#106)에 닫혔다. 그런데 이 행은 저작을 마친 뒤 **세 번째 차단**을 만났다 — 전용 e2e `resource_generic_ac11.py` 를 저작해 CI 에서 처음 실물로 돌린 결과, **성공한 exec 가 실패로 보고된다**(`rct_20260918-0003` 시도 2, run 35368945500 실측: `echo` 가 stdout 을 정상으로 돌려주는데 응답이 `exitCode: null · success: false`, 따라서 `isError: true`). 원인은 `internal/k8s/exec.go` 의 `ExecResource` 다 — 종료 코드를 **`StreamWithContext` 가 돌려준 에러에서만** 뽑고(`utilexec.CodeExitError`), 명령이 0 으로 끝나면 그 에러가 `nil` 이라 `exitCode` 가 세팅되지 않는다. 그 결과 `Success: exitCode != nil && *exitCode == 0` 은 **성공했을 때 결코 참이 될 수 없다**. Go 단위 `resource_test.go::TestExecStreamsAndCaps` 는 실패 경로(`ExitCode: 1 · Success: false`)만 단언해 이 자리를 보지 못했고, **실물 왕복이 처음으로 그것을 관측했다.** 이 결함은 시나리오 11 의 나머지 단언까지 공허하게 만든다 — `isError` 가 항상 참이므로 「`container` 누락이 거부된다」가 거부를 증명하지 않는다. | **docs-impl(제품 수정)** → 그 뒤 이 렌즈(e2e 재저작) | ⑶ `internal/k8s/exec.go` 가 성공한 exec 에 `exitCode: 0 · success: true` 를 싣도록 고칠 것(`streamErr == nil` 갈래 신설). 그 뒤 `resource_generic_ac11.py` 를 다시 저작하면 이 행이 닫힌다 — 파일 자체는 `rct_20260918-0003` 시도 2 가 이미 써서 픽스처 대기까지 통과시켰고(그 판본은 PR #132 에서 이 결함 때문에 빠졌다), 남은 것은 제품 수정 하나다 |
...
| **test-resource-generic.md#시나리오 17** | (a) 의 `kind=Secret` `resource_list` 와 (c')·(d) 가 전제하던 `nodes/proxy`·`secrets` 의 `watch` 는 2026-09-14 RBAC 개정으로 전부 권한 안에 들어왔다. (d) 의 승인 갈래도 실물 gatekeeper 픽스처(#106)가 착지해 태울 수 있다. **남은 것은 (c) 하나이고, 그 문면은 이 슬라이스에서 실측으로 좁혔다** — 직전 판은 스트림 셋을 「`attach`·`port_forward`·`proxy` 가 미등록」으로 적었으나 `attach` 는 2026-09-18 에 #127 로 등록됐다(`internal/mcp/attach.go`·`internal/k8s/attach.go`·`gate.go` 의 `create on pods/attach`·`toolslist.go`·`EXPECTED_TOOLS`). 지금 미등록인 것은 **`resource_port_forward` 와 `resource_proxy` 둘뿐**이다(레포 전체 grep 이 `docs/` 밖에서 0 히트 — `exec` 은 2026-09-17, `attach` 는 2026-09-18 등록). 셋 중 둘이 여전히 없으므로 **등재 자체는 유효하고 계수는 움직이지 않는다.** | docs-impl(도구) → 이 렌즈(e2e) | ⑴ `resource_port_forward`·`resource_proxy` 등재 — **`attach` 는 더는 차단 요인이 아니다**. ⑵ 실물 gatekeeper 픽스처는 해소됨. 난수 토큰 Secret 과 그 토큰을 서빙하는 HTTP 파드 픽스처는 이 렌즈 자신의 저작이라 차단 요인이 아니다 |
...
|------|-----------|-----------|-----------|
| 2026-09-19 | **`test-resource-generic.md#시나리오 13`(실행 중 컨테이너 stdio 접속)을 전용 e2e 저작으로 닫았다 — 「구현 대기」가 아니라 **규칙 1의 공백**이었다.** ⚠️ 이 행의 `rct_20260918-0004` 는 **`tbm_homelab-k3s-mcp-scenario-e2e`** 의 것이다 — 바로 아래 2026-09-18 행의 같은 id 는 자매 모델 `tbm_homelab-k3s-mcp-docs-impl` 소관이고 task id 는 모델 안에서만 유일하다. ⑴ **왜 저작인가**: #127 이 `resource_attach` 를 착지시키며 이 행의 마지막 선행을 스스로 해소했는데도 행이 ⏳ 에 남았고, 해제 조건 칸이 스스로 **「전용 e2e 파일 저작만 남았다」**고 적었다. 규칙 6 은 「하네스가 그 상태에 **도달할 수 없을** 때」의 임시 등재이고 규칙 4 가 「아직 자동화하지 못했다」를 사유에서 배제하므로, 관측 대상이 실재하는 지금 그것은 정의상 규칙 1 의 공백이다 — 시나리오 1·3·5 를 닫은 것과 같은 판정이다(근거를 고쳐 쓴 시나리오 8 계열이 아니다). ⑵ **게이트가 이 축을 원리적으로 못 본다**: `check_ac_mapping.py` 는 행의 제목·`⏳` 표시·집계 산술만 대조하고 **행의 사유가 코드에 비추어 참인지는 검사하지 않아**, 행이 표에 남아 있는 한 개수가 움직이지 않고 등재 절 지문(`reg:`)도 저절로는 이동하지 않는다 ⇒ 방치하면 영구히 초록인 어긋남이다. ⑶ **이 층이 더하는 것**(Go 단위 `internal/mcp/attach_test.go` 가 이미 덮는 것 위에): 「새 프로세스가 뜨지 않았다」를 **파드 로그 대조**로 세운다 — attach 가 돌려준 틱 줄이 `kubectl logs` 에도 그대로 있으면 그것은 컨테이너 stdout 으로 간 줄, 즉 원래 살아 있던 프로세스의 스트림이고, exec 였다면 그 호출의 스트림에만 있다. 틱 번호가 1 에서 시작하지 않는다는 것이 같은 사실의 둘째 증인이다. `readSeconds=31` 은 거부를 확인하는 것으로 끝내지 않고 **호출 앞뒤의 gatekeeper 요청 수가 같은지**를 함께 본다 — 승인을 한 번 태우고 나서 오는 거부는 시나리오가 요구한 그 거부가 아니기 때문이다(`attachPairs` 가 인자 검사를 쌍 해석 앞에 두는 이유). stdin 은 토큰의 되울림을 응답과 파드 로그 **양쪽**에서 찾는다. ⑷ **대상 파드는 e2e 자신이 세운다** — `stdin: true`(그리고 `stdinOnce: false` — 켜면 첫 접속이 끊길 때 stdin 이 닫혀 셋째 실행 단계가 성립하지 않는다)가 필요한 파드는 이 시나리오 전용이라 `tests/k8s/kind/` 에 두면 아무도 쓰지 않는 픽스처가 CI 에 상주한다. 「각 파일이 자기 선행 조건을 스스로 성립시킨다」가 이 레포가 이미 택한 방향이고(#132 도 픽스처·`ci.yml` 을 건드리지 않았다), 그래서 **`ci.yml` 무변경**이다 — `run_all.py` 가 primary 그룹에 자동 배차한다. ⑸ **함께 고친 문면 1**: 시나리오 17 행의 근거가 스트림 셋을 「`attach`·`port_forward`·`proxy` 미등록」으로 적었는데 `attach` 는 #127 로 등록됐다. 남은 차단(`port_forward`·`proxy` 가 `docs/` 밖 grep 0 히트)이 여전히 유효하므로 **행은 구현 대기에 남기고 근거만** 실측으로 다시 썼다 — 그래서 **이 단계에서 집계는 움직이지 않는다**(움직였다면 오류 신호다). ⑹ 숫자는 손으로 짓지 않고 `check_ac_mapping.py` 출력에서 뽑았고, 신규 파일이 들여온 주석 14줄·docstring 19줄은 같은 PR 에서 판정해 `docs/comment-policy/ledger.md` 두 원장에 등재했다(제거 4줄 — ① 선언 재진술 2 · ② 살아 있는 주석 재진술 2). **범위 밖(근거와 함께)**: 남은 구현 대기 20행 — 도구 미등록(`port_forward`·`proxy`·`delete_collection`)은 `tbm_homelab-k3s-mcp-docs-impl` 몫이고, #132 가 그 모델로 인계한 시나리오 11 의 exec 종료 코드 결함도 그쪽이다. 모델 정의의 매칭 단위 제외 목록이 `run_all.py` 를 빠뜨린 문면(tobe-modeler 몫, 9연속 관측), 레지스트리 요약 표의 `(규칙 6 — 도구 미구현)` 라벨 부정확(체커 비대조 칸, 부모에서 이미 있던 별건). 판정 근거는 `rct_20260918-0004` | 시나리오 전집 85 · 예외 1 · **구현 대기 21** · 1:1 대상 63 · 매칭 파일 63 · 공백 0 (시나리오 13 은 ⏳) | 시나리오 전집 85 · 예외 1 · **구현 대기 20** · 1:1 대상 **64** · 매칭 파일 **64** · 공백 0 (시나리오 13 은 ✅ `resource_generic_ac13.py`), 불변식 `85 − 1 − 20 = 64 = 64` |
