...
| **test-resource-generic.md#시나리오 14** | `resource_port_forward` 가 등록돼 있지 않고(레포 전체 grep 0 히트) `pods/portforward` 도 `k8s/rbac.yaml` 의 ClusterRole 밖이다. HTTP 를 서빙하는 파드 픽스처도 없고, 「같은 승인 id 로 두 번째 왕복」을 재려면 실물 승인이 필요한데 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. | rbac 소유자 판단 → docs-impl(도구) → 이 렌즈(e2e) | ⑴ `pods/portforward` 부여, ⑵ `resource_port_forward` 등록, ⑶ HTTP 서빙 파드 픽스처, ⑷ 실물 gatekeeper 픽스처 |
| **test-resource-generic.md#시나리오 15** | `resource_proxy` 가 등록돼 있지 않고(레포 전체 grep 0 히트) `pods/proxy`·`services/proxy`·`nodes/proxy` 가 전부 `k8s/rbac.yaml` 의 ClusterRole 밖이다. 네 호출 모두 승인을 요구하는데 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. | rbac 소유자 판단 → docs-impl(도구) → 이 렌즈(e2e) | ⑴ 세 `*/proxy` 쌍 부여, ⑵ `resource_proxy` 등록, ⑶ HTTP 파드·Service 픽스처, ⑷ 실물 gatekeeper 픽스처 |
