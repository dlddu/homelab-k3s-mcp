...
### 시나리오 레지스트리 (85) — ✅ 전용 파일 64 · ⬜ 분할 대기 0 · ⏳ 구현 대기 20 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **85 − 1(예외) − 20(구현 대기) = 64 = 매칭 파일 64**, 공백 **0**.
...
| test-resource-generic.md#시나리오 11 | 컨테이너 안에서 명령 실행 | ⏳ 구현 대기 (규칙 6 — 아래 행별 근거) |
...
| **test-resource-generic.md#시나리오 11** | **⑴ 도구 축은 2026-09-17 에 닫혔고 ⑵ 픽스처 축은 2026-09-18(#106)에 닫혔다. ⑶ **세 번째 차단이던 제품 결함은 2026-09-18 `rct_20260918-0005` 에서 닫혔다.**  그 결함이 무엇이었나: 전용 e2e `resource_generic_ac11.py` 를 저작해 CI 에서 처음 실물로 돌린 결과 **성공한 exec 가 실패로 보고됐다**(`rct_20260918-0003` 시도 2, run 35368945500 실측: `echo` 가 stdout 을 정상으로 돌려주는데 응답이 `exitCode: null · success: false`, 따라서 `isError: true`). 원인은 `internal/k8s/exec.go` 의 `ExecResource` 가 종료 코드를 **`StreamWithContext` 가 돌려준 에러에서만** 뽑은 것이다 — 명령이 0 으로 끝나면 그 에러가 `nil` 이라 `exitCode` 가 세팅되지 않아 `Success: exitCode != nil && *exitCode == 0` 이 **성공했을 때 결코 참이 될 수 없었다**. **수정 슬라이스가 같은 자리에서 두 번째 결함을 하나 더 찾아 함께 닫았다**: 바이트 상한이 차면 `limitedWriter.onFull = cancel` 이 컨텍스트를 **취소**하는데, 그 취소는 `DeadlineExceeded` 가 아니라 `Canceled` 라 분류의 `default` 로 떨어져 **응답 대신 `APIError`** 가 됐다 — AC12 가 계약한 「상한을 넘기는 출력이 잘리고 잘렸음이 응답에 표시된다」가 이 도구에서는 서지 않았다는 뜻이고, 이 시나리오의 **`yes` 단계가 정확히 그 자리**다. 둘 다 분류가 `ExecResource` 안에 묻혀 단위 테스트가 닿지 못한 것이 원인이라, 수정은 분류를 순수 함수 `execStreamOutcome` 으로 뽑아 `exec_test.go::TestExecStreamOutcome` 이 여섯 갈래를 단언하게 했다. **남은 잔여는 e2e 재저작 하나이고 소관이 이 렌즈로 넘어갔다.** | 이 렌즈(e2e 재저작) — docs-impl 의 제품 수정 선행은 충족됨 | `resource_generic_ac11.py` 를 다시 저작하면 이 행이 닫힌다 — 파일 자체는 `rct_20260918-0003` 시도 2 가 이미 써서 픽스처 대기까지 통과시켰고(그 판본은 PR #132 에서 이 결함 때문에 빠졌다), 이제 선행 제품 수정이 착지했으므로 **남은 것은 저작뿐**이다. 재저작 시 `echo` 왕복은 `exitCode: 0 · success: true · isError: false` 를, `yes` 왕복은 **에러가 아닌 응답**에 `stdoutTruncated: true` 를 단언할 것 — 그 둘이 이번에 닫힌 자리다 |
