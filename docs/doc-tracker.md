...
| **test-resource-generic.md#시나리오 8** | `resource_update` 가 등록돼 있지 않다(레포 전체 grep 0 히트). `update` 와 `⟨kind⟩/scale` 은 2026-09-14 RBAC 개정으로 권한 안에 들어왔으나, 각 단계가 승인을 요구하는데 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ `resource_update` 등록 **그리고** ⑵ 실물 gatekeeper 픽스처 |
