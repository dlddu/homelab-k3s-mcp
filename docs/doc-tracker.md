...
- **문서 선행(미구현)**: `github_commit_status_create`(PRD AC6개)는 2026-09-19 문서만 섰다 — 등록 도구 수 21종에 들어가지 않으며, 그 PRD의 시나리오 6개는 아래 「⏳ 구현 대기」에 등재했다. 클러스터 밖(GitHub) 쓰기이므로 게이트 대상·게이트 밖 예외 어느 계수에도 들어가지 않는다(`prd-github-commit-status` 「범위 밖」). **`github_app_installation_token` AC5(statuses 쓰기 배제 · 읽기는 발급)는 문서 선행에서 빠졌다** — 구현이 `internal/github` 에 착지했다(`rct_20260918-0004`). AC5는 새 도구가 아니라 기존 도구의 발급 규칙이므로 등록 도구 수는 21종 그대로다. 이로써 `prd-github-commit-status` 「배포 순서(필수)」의 ①이 닫혔고, 남은 선행은 ②(GitHub UI 에서 App 에 `Commit statuses: Read and write` 추가 — 사람 작업)다
...
| 허브 도달 가능 문서 | ✅ **45 / 45** (가치 1 + PRD 15 + 테스트 15 + 상태 추적 1 + 정책 13), 끊긴 링크 0 |
...
### 시나리오 레지스트리 (85) — ✅ 전용 파일 65 · ⬜ 분할 대기 0 · ⏳ 구현 대기 19 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **85 − 1(예외) − 19(구현 대기) = 65 = 매칭 파일 65**, 공백 **0**.
...
| test-resource-generic.md#시나리오 13 | 실행 중 컨테이너 stdio 접속 | ⏳ 구현 대기 (규칙 6 — 제품 결함) |
...
| **test-resource-generic.md#시나리오 13** | **⑴·⑵ 두 선행은 2026-09-18 에 충족됐다** — `resource_attach` 가 등록·구현됐고(`internal/k8s/attach.go` · `internal/mcp/attach.go` · `gate.go` 의 `create on pods/attach` 선언 · `toolslist.go`), 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`·`tests/k8s/kind/gatekeeper-variant.yaml`)도 PR #106 으로 착지했다. 그래서 이 행은 「남은 것은 선행이 아니라 저작이다」로 서 있었는데, **저작해서 CI 에 태워 보니 세 번째 차단이 드러났다** — `rct_20260918-0004` 시도 1(PR #139, head `aeb7d4d`, run 35408563767). **`resource_attach` 는 `stdin` 이 주어지면 `readSeconds` 창을 지키지 않는다.** 같은 파드·같은 `readSeconds=3` 인데 두 접속의 실측 수명이 다르다(서버 로그): stdin 없는 접속은 승인 `00:16:21.991` → 응답 `00:16:24.993` 로 **3.00초**를 붙어 있으면서 틱을 돌려줬고, stdin 을 실은 접속은 승인 `00:16:27.127` → 응답 `00:16:27.143` 로 **0.016초** 만에 `stdout: ""` 을 돌려줬다. 그것도 에러가 아니라 **성공 응답**이라(`isError:false`·`stdinWritten:true` 는 둘 다 통과했다) 아무도 실패로 세지 않는다. 자리는 `internal/k8s/attach.go` 가 `opts.Stdin = strings.NewReader(*stdin)` 으로 **즉시 EOF 가 되는 리더**를 넘기는 것이다 — client-go 의 remotecommand 는 stdin 복사가 끝나면 원격 stdin 스트림을 닫고, 서버는 그 닫힘을 접속 종료로 읽어 창이 끝나기 전에 스트림을 걷는다. **그래서 「`stdin` 이 대상 프로세스에 전달되어 그 반응이 출력에 나타남」이라는 기대 결과에 이 하네스는 도달할 수 없다** — 16ms 창에는 되울림이 들어올 자리가 없다. 이것이 규칙 6 이 말하는 「하네스가 시나리오가 기술하는 상태에 도달할 수 없다」이고, **시나리오 11 이 2026-09-18 에 지나온 것과 같은 형태**다(저작 → 실물 왕복이 제품 결함 관측 → 파일 물림 → docs-impl 이 수정(#137) → 이 렌즈가 재저작). 리베이스·픽스처 수정으로는 닫히지 않는다는 것도 같은 실측이 말한다 — 두 접속을 하나로 합쳐도 그 하나가 stdin 을 실은 접속이라 16ms 로 걷히고, 파드 쪽 stdin 루프를 어떻게 고쳐도 16ms 안에 되울림을 밀어 넣을 수는 없다 | docs-impl(제품 수정) → 그 뒤 이 렌즈(e2e 저작) | ⑴ `internal/k8s/attach.go` 가 **`stdin` 이 주어져도 `readSeconds` 창을 끝까지 지키도록** 고칠 것. AC13 은 「`readSeconds`(기본 5, 상한 30) 동안 출력을 모아 반환하며, `stdin` 이 주어지면 붙은 직후 한 번 써 넣는다」를 **둘 다** 계약하는데 지금은 둘째가 첫째를 무효로 만든다. 페이로드를 한 번 흘린 뒤 **창의 `context` 가 끝날 때까지 EOF 를 미루는 리더**로 바꾸면 「한 번만 써 넣는다」는 그대로 유지된다(대상 프로세스가 EOF 대신 침묵을 보게 되는 것이 차이인데, AC13 도 PRD 도 EOF 를 계약하지 않는다). 재발 방지 단언은 **그 리더 자체**에 걸 것 — 분류를 `AttachResource` 안에 두면 SPDY 실행기 없이는 단위가 닿지 못한다(`execStreamOutcome` 선례). ⑵ 그 뒤 전용 e2e `resource_generic_ac13.py` 저작 — **판본은 이미 있다**(PR #139 의 `aeb7d4d`). 세 케이스 중 ⑴「3초 창이 기존 프로세스의 스트림임을 파드 로그 대조와 틱 번호>1 로 증명」·⑵「`readSeconds=31` 이 승인 요청 0건 증가로 거부」는 실물에서 **통과**했고, ⑶「stdin 의 되울림」만 이 결함에 걸렸다. 재저작은 그 판본을 그대로 들고 오면 되고, 그때 `(미작성)` 표기(규칙 7)와 이 행의 철거를 같은 PR 에서 할 것 |
