...
| grafana-token/AC3 | 미설정 시 graceful 거부 | 🚫 예외 후보 |
| grafana-token/AC4 | 발급자 토큰 비노출 | 🚫 예외 후보 |
...
| opensearch-search/AC4 | 미설정 시 graceful 거부 | 🚫 예외 후보 |
...
| platform-auth-safety/AC4 | 하드닝된 런타임 | 🚫 예외 후보 |
...
| workload-restart/AC2 | 파괴적 작업 표기 | 🚫 예외 후보 |
| workload-scale/AC1 | 레플리카 설정 | ✅ 통합 `workload.py` |
| workload-scale/AC2 | DaemonSet 거부 | ✅ 통합 `workload.py` |
| workload-scale/AC3 | 파괴적 작업 표기 | 🚫 예외 후보 |

### ⬜ e2e 보강 backlog (7) — e2e 가능 클러스터 동작, 전용 케이스 신설 필요
...
### 🚫 e2e 예외 후보 (13) — e2e 비현실적, 모델 정의 예외 개정 제안

> 아래는 e2e로 커버하기 비현실적인 AC의 **제안 목록**이다. definition이 `task에서 제안한다`고 명시하므로 모델 정의(`to-be-models.json`)에 일방 적용하지 않고, ratify 후 정의의 예외 목록에 반영한다. 각 항목은 대체 검증 수단을 갖는다.

- **aws-config-get/AC3** 미설정 시 graceful 거부 — [미설정 거부] 대상 시크릿 없이 배포한 상태를 단일 통합 클러스터에서 재현하기 비현실적(픽스처가 모든 시크릿 제공). 대체: Go 단위(missing-env→graceful error); 필요 시 no-config 배포 변형으로 e2e 승격 가능.
- **dear-baby-reset-user/AC3** 파괴적 작업 표기 — [파괴적 표기] 도구 스키마의 destructive 메타데이터 속성 — 클러스터 동작이 아니라 도구 정의 속성. 대체: Go 단위(도구 정의 destructive 단언).
- **github-app-installation-token/AC3** 미설정 시 graceful 거부 — [미설정 거부] 위와 동일(no-config 상태 재현 비현실적). 대체: Go 단위(missing-env→graceful error).
- **grafana-token/AC3** 미설정 시 graceful 거부 — [미설정 거부] 위와 동일. 대체: Go 단위(missing-env→graceful error).
- **grafana-token/AC4** 발급자 토큰 비노출 — [출력 비노출] 응답에 발급자 토큰이 없다는 출력-내용 속성 — 단위가 자연. 대체: Go 단위(출력 비노출 단언).
- **opensearch-document-delete/AC3** 파괴적 작업 표기 — [파괴적 표기] 도구 정의 destructive 속성. 대체: Go 단위.
- **opensearch-document-delete/AC5** 미설정 시 graceful 거부 — [미설정 거부] no-config 상태 재현 비현실적. 대체: Go 단위.
- **opensearch-document-put/AC3** 파괴적 작업 표기 — [파괴적 표기] 도구 정의 destructive 속성. 대체: Go 단위.
- **opensearch-document-put/AC5** 미설정 시 graceful 거부 — [미설정 거부] no-config 상태 재현 비현실적. 대체: Go 단위.
- **opensearch-search/AC4** 미설정 시 graceful 거부 — [미설정 거부] no-config 상태 재현 비현실적. 대체: Go 단위.
- **platform-auth-safety/AC4** 하드닝된 런타임 — [정적 매니페스트] k8s/deployment.yaml securityContext 정적 검증 — 정의가 든 e2e 예외 예시. 대체: 정적 매니페스트 리뷰 + (선택) 런타임 securityContext 단언 단위.
- **workload-restart/AC2** 파괴적 작업 표기 — [파괴적 표기] 도구 정의 destructive 속성. 대체: Go 단위.
- **workload-scale/AC3** 파괴적 작업 표기 — [파괴적 표기] 도구 정의 destructive 속성. 대체: Go 단위.

## 변경 이력

| 시점 | 변경 내용 | 이전 상태 | 이후 상태 |
|------|-----------|-----------|-----------|
| 2026-07-12 | AC↔e2e 1:1 정합성(reconciler) 레지스트리 신설: e2e-only 렌즈로 52 AC 분류(통합 32·보강 7·예외 후보 13), per-AC 케이스 식별 규약 명문화, e2e 보강 backlog·예외 후보 목록 작성. 전용 per-AC 케이스 분리·신설(통합 스크립트 리팩터)과 모델 정의 예외 개정은 후속·ratify 대상. | 통합 파일 7개 다중 AC 공유, 인코드 AC 선언 1건, e2e 케이스 규약 부재 | 52 AC e2e 렌즈 분류 완료, 규약·backlog·예외 제안 문서화(tests/ 코드 미변경) |
