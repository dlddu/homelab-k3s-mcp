...
- 등록 도구: **17종** (`internal/mcp/toolslist.go` = `tests/integration/_helpers.py::EXPECTED_TOOLS`) — 그중 게이트 대상 쓰기 도구는 `resource_patch` 1종
- **건강 상태**: 🟡 **위험 있음** — 문서 계층의 연결은 모두 이어져 있으나,
  ⑴ 폐기된 6종 중 **2종**(`workload_restart`·`workload_scale`)이 아직 코드에 살아 있고
...
| **test-approval-gate.md#시나리오 1** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. 게이트 대상 다섯(`resource_apply`·`resource_delete`·`resource_exec`)과 비게이트 셋(`resource_scale`·`resource_restart`·`kind=ConfigMap` 의 `resource_get`)을 **같은 시나리오에서** 대조해야 하는데 앞의 셋이 미등록이고(레포 전체 grep 0 히트), `configmaps` 는 `k8s/rbac.yaml` 의 ClusterRole 밖이다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 해당 쓰기·스트림 도구가 등록되고 ⑵ `configmaps` 의 `get` 이 부여되면 해제된다. 실물 gatekeeper 는 이 시나리오엔 불필요하다 — `PENDING` 유지 스텁으로 족하다 |
| **test-approval-gate.md#시나리오 2** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. 요청 본문을 두 번 만들려면 게이트 대상 도구를 두 번 불러야 하는데 그 도구가 하나도 등록돼 있지 않다(레포 전체 grep 0 히트). | docs-impl(도구) → 이 렌즈(e2e) | 게이트 대상 쓰기 도구가 하나라도 등록되면 해제된다(gatekeeper 는 본문을 기록하는 스텁으로 족하다) |
| **test-approval-gate.md#시나리오 3** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. **게이트 대상 verb 각각**의 `context` 를 모아야 한다 — `create`·`update`·`patch`·`delete`·`deletecollection`·`exec`·`attach`·`port_forward`·`proxy` 아홉이 전부 미등록이다(레포 전체 grep 0 히트). | docs-impl(도구) → 이 렌즈(e2e) | 아홉 verb 를 행사하는 도구가 모두 등록되면 해제된다(부분 등록으로는 「verb 각각」이 성립하지 않는다) |
| **test-approval-gate.md#시나리오 4** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. `PENDING`→`APPROVED` 전이와 `EXPIRED` 관측을 실물로 태워야 하는데, 게이트 대상 도구가 미등록이고 승인·거절을 사람 대신 태울 실물 gatekeeper 픽스처도 없다(`tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다). | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 게이트 대상 도구 등록 **그리고** ⑵ 실물 gatekeeper 픽스처. 스텁으로는 이 시나리오가 겨냥하는 실제 폴링을 관측할 수 없다 |
| **test-approval-gate.md#시나리오 5** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. 8가지 실패 경로를 재현하려면 게이트 대상 도구가 있어야 하고(미등록), 대조군인 「같은 조건에서 `resource_list` 와 비게이트 `resource_get` 은 정상 동작」의 절반만 현재 관측 가능하다. | docs-impl(도구) → 이 렌즈(e2e) | 게이트 대상 쓰기 도구가 등록되면 해제된다(gatekeeper 는 경로별 스텁으로 족하다) |
| **test-approval-gate.md#시나리오 6** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. TOCTOU 를 태울 `resource_patch`·`resource_update(subresource=scale)`·`resource_exec` 이 미등록이고, `kind=Secret` 의 `resource_get` 갈래는 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인 뒤에도 값을 얻지 못한다. 승인·거절을 사람 대신 태울 실물 gatekeeper 픽스처도 없다(`tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다). | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 세 쓰기 도구 등록, ⑵ `secrets` 의 `get` 부여(두 PRD 충돌 해소 이후), ⑶ 실물 gatekeeper 픽스처 |
| **test-approval-gate.md#시나리오 7** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. 승인을 **써 버리는** 실행이 필요한데 게이트 대상 도구가 미등록이고, 「같은 Secret 재조회가 새 승인 요청을 만든다」 갈래는 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이다. 승인·거절을 사람 대신 태울 실물 gatekeeper 픽스처도 없다(`tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다). | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 게이트 대상 도구 등록, ⑵ 실물 gatekeeper 픽스처. Secret 갈래까지 닫으려면 ⑶ `secrets` 부여가 추가로 필요하다 |
| **test-approval-gate.md#시나리오 8** | 게이트를 **실제로 태울 수 있는** 호출은 현재 `resource_get(kind=Secret)` 하나뿐이다 — `list` 는 원리상 게이트 밖이고 `get` 은 민감 종류일 때만 탄다(`internal/mcp/gate.go:174-189`, 기본 `RESOURCE_GATED_KINDS=v1/Secret`). 그마저 `secrets` 가 `k8s/rbac.yaml` 의 ClusterRole 밖이라 승인이 떨어져도 apiserver 에 닿지 않는다. 감사 로그의 두 경우(승인 실행 1회·거절 1회)를 만들려면 승인 흐름을 끝까지 태워야 한다 — 게이트 대상 도구가 미등록이고 승인·거절을 사람 대신 태울 실물 gatekeeper 픽스처도 없다(`tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다). | docs-impl(도구) → 이 렌즈(e2e) | ⑴ 게이트 대상 도구 등록 **그리고** ⑵ 실물 gatekeeper 픽스처(거절은 스텁으로도 되지만 `processedById` 는 실물이라야 남는다) |
