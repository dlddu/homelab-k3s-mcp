...
### 시나리오 레지스트리 (78) — ✅ 전용 파일 48 · ⬜ 분할 대기 0 · ⏳ 구현 대기 29 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **78 − 1(예외) − 29(구현 대기) = 48 = 매칭 파일 48**, 공백 **0**.
...
| test-resource-generic.md#시나리오 3 | 절단과 이어보기 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 미작성) |
...
| **test-resource-generic.md#시나리오 3** | 🟢 **2026-09-14 — 마지막 살아 있던 차단 요인이 죽었다.** 직전 근거였던 「기대 결과의 `limit=1000` 은 500 으로 **강제 하향** 이 구현과 어긋난다」는 `rct_20260914-0015` 가 **시나리오 문면을 거부로 정정해** 해소했다 — 그 행이 인용하던 「`internal/k8s/resource.go` 주석이 거부를 의도로 명시한다」는 **실측 결과 사실이 아니었고**(그 주석은 `// ListDefaultLimit and ListMaxLimit are AC3's bounds.` 한 줄로 둘 중 무엇인지 말하지 않는다), 대신 방향을 정한 것은 셋이다: ⑴ 같은 PRD 의 **AC5 가 자매 상한 `tailLines` 에 「클램프하지 않고 **거부**한다」를 이미 축자로 정한다**(어긋남은 SSOT 내부의 불일치였다), ⑵ `internal/mcp/toolslist.go` 가 도구 스키마에 `"maximum": 500` 을 선언한다(클램프로 가려면 **공개 계약면의 기계 검증 제약을 지워야** 한다), ⑶ `internal/server/mcp_test.go::TestResourceListLimitDefaultsAndCeiling` 이 「클램프는 「호출이 통과했다」류 단언을 그대로 통과시키고 호출자는 덜 받았다는 사실을 끝내 모른다」를 주석으로 적고 `limit=501` 거부를 단언한다 — **거부는 이미 기계로 고정돼 있었고 문서만 뒤처져 있었다.** 120개 ConfigMap 픽스처 저작은 이 렌즈 자신의 트리 안이다. | 이 렌즈(e2e 저작) | 전용 e2e `resource_generic_ac3.py` 저작 + 120개 ConfigMap 픽스처. **docs-impl 쪽 차단 요인은 0이다** — 규칙 4 가 「아직 자동화하지 못했다」를 예외 사유에서 배제하므로 이 행은 정의상 **규칙 1 의 공백**이고, 저작이 끝나면 표에서 빠지며 1:1 계수로 복귀한다 |
