...
통합 테스트는 기존 `github-mock`(`tests/k8s/kind/github-mock.yaml`)을 쓴다. 그 스텁의 대상은 현재
토큰 발급 엔드포인트 하나이므로, 이 문서의 시나리오를 자동화하려면 **`POST
/repos/{owner}/{repo}/statuses/{sha}`와 받은 요청을 돌려주는 기록 엔드포인트**를 스텁에 더하고
...
- **검증 AC**: AC1
- **자동화**: (미작성) — Go 단위 `TestCommitStatusCreatesStatus`,
  `TestCommitStatusSurfacesGitHubError` 계획. 통합 `github_commit_status_ac1.py` 계획
...
- **검증 AC**: AC2
- **자동화**: (미작성) — Go 단위 `TestCommitStatusMintsNarrowToken`,
  `TestCommitStatusNeverReturnsToken` 계획. 통합 `github_commit_status_ac2.py` 계획
...
- **검증 AC**: AC3
- **자동화**: (미작성) — Go 단위 `TestCommitStatusValidatesBeforeGitHub`(표 기반 6행) 계획.
  통합 `github_commit_status_ac3.py` 계획
...
- **검증 AC**: AC4
- **자동화**: (미작성) — Go 단위 `TestCommitStatusRejectsForeignContext`,
  `TestCommitStatusRefusesWhenNoPrefixConfigured` 계획. 통합 `github_commit_status_ac4.py` 계획
  (b는 env 한 줄이 다른 배포 변형이 필요하다)
...
- **검증 AC**: AC5
- **자동화**: (미작성) — Go 단위 `TestCommitStatusUnavailableReturnsToolError` 계획. 통합
  `github_commit_status_ac5.py` 계획
...
- **기대 결과**: `github_commit_status_create`의 `readOnlyHint=false`, `destructiveHint=false`,
  `openWorldHint=true`
- **검증 AC**: AC6
- **자동화**: (미작성) — 통합 `github_commit_status_ac6.py` 계획
