...
- **설명**: 각 도구는 자신이 행사하는 `(verb, resource[/subresource])` 쌍을 선언하고,
  ...
  「도구가 무엇을 행사하는가」는 핸들러가 실행돼야 드러나므로, 이 약속은 두 겹으로 지킨다.
  - **기동 시 — 선언의 존재를 강제한다.** 등록된 모든 도구는 ⑴ 자신이 행사하는 쌍(고정 쌍,
    또는 호출 인자에서 쌍을 도출하는 규칙) 또는 ⑵ 「쿠버네티스 자원 권한을 행사하지 않는다」는
    **명시적 표식과 그 사유** 중 정확히 하나를 가진다. discovery(`api_resources`)는 자원 권한이
    아니므로 ⑵의 한 종류로 표식에 적는다. 둘 다 없거나 둘 다 있는 도구가 하나라도 있으면
    **기동이 실패한다.** 「빈 선언」은 ⑵가 아니다 — 선언을 빠뜨린 도구와 아무것도 행사하지 않는
    도구가 구조상 구별돼야 빠뜨림이 조용히 통과하지 않는다.
  - **호출 시 — 선언 밖 행사를 막는다.** 핸들러가 받는 쿠버네티스 클라이언트는 **그 호출에서
    선언(도출)된 쌍으로 범위가 제한된다.** ⑵ 표식 도구의 클라이언트는 표식이 허락한 것
    (discovery 표식이면 discovery) 외에는 아무것도 할 수 없다. 선언 밖의 쌍을 행사하려는 요청은
    **apiserver 에 보내지기 전에** 거부되고 그 도구 호출은 실패로 끝난다. 게이트 자신의 사전
    읽기(AC11)는 도구 선언이 아니라 AC11 의 표로 범위가 정해지므로 이 제한과 별개다.

  기동 검사만으로는 「선언은 있으나 틀린」 도구를 잡지 못하고, 호출 제한만으로는 한 번도 호출되지
  않은 도구의 빠뜨림을 드러내지 못한다. 그래서 둘 다 둔다.
- **달성 가치**: V3
- **검증 방법**: 변경 verb 여섯과 민감 종류의 `get`·`watch`를 승인 없이 호출하면 쿠버네티스
  클라이언트가 단 한 번도 호출되지 않는다(가짜 k8s 서비스의 호출 카운트가 0). `list`와
  비게이트 종류의 `get`은 같은 조건에서 정상 동작한다. 쌍도 표식도 없는 가짜 도구를 등록하면
  기동이 실패한다. 쌍 A 만 선언하고 쌍 B 를 행사하는 가짜 도구를 (승인이 있더라도) 호출하면
  B 에 대한 쿠버네티스 호출 카운트가 0 이고 도구 호출은 실패한다. 출하되는 모든 도구는 기동
  검사를 통과하고, 각 도구의 정상 호출이 호출 제한에 걸리지 않는다(선언이 실제 행사와 같다).
