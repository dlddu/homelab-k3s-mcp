...
- **검증 AC**: AC1
- **자동화**: 통합 `grafana_token_ac1.py::test_grafana_token_ac1_read_only_short_lived` —
...
- **검증 AC**: AC2
- **자동화**: 통합 `grafana_token_ac2.py::test_grafana_token_ac2_ready_to_use_env` — 메트릭·로그
...
- **자동화**: Go 단위 `mcp_test.go::TestGrafanaTokenUnavailableReturnsToolError`. 구성 검증은
  ...
  `tests/integration/grafana_token_ac3.py::test_grafana_token_ac3_unconfigured_refusal`
...
- **검증 AC**: AC4
- **자동화**: 통합 `grafana_token_ac4.py::test_grafana_token_ac4_issuer_token_not_exposed` — 발급 응답 .env에
