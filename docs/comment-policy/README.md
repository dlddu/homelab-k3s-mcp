...
- **두 표면 모두 잔량 래칫이 걸려 있다**(줄 주석 R4, docstring R8): `합계 + 잔량 == 실측 전체`.
  **원장 행의 재측정만으로는 새 범위가 보이지 않기 때문이다** — R1~R3·R5~R7 은 등재된 행의
  범위만 재므로, 어느 행에도 없는 파일은 원리적으로 검사 대상이 아니다. 줄 주석 쪽 래칫은
  2026-09-13 에 뒤늦게 세웠고, 그 계기는 새 파일 넷이 주석 192줄을 들고 들어왔는데 게이트가
  rc=0 이었던 사건이다(`ledger.md`의 잔량 마커 옆 경위 참조).
- **판정이 *무엇을 물었는지*에도 래칫이 걸려 있다**(R9): 위 여덟 규칙은 등재 범위가 *변했는지*만
  재고 그 판정이 복원 경로 넷 중 **어디까지 물었는지**는 보지 않는다. 그래서 ①②만 검토한 판정도
  rc=0 으로 착지했다(그 축은 2026-09-25 의 R11·R12 가 따로 닫는다 — 아래). R9 는 결과 칸이 `복원경로 ③:`·`복원경로 ④:` 를 **둘 다** 담고
  「복원경로 ③④ 판정 @지문 \<12자\>」 앵커의 지문이 **그 행의 현재 지문과 같을 때만** ③④ 판정
  완료로 세고, `완료 행 수 + 잔량 == 전체 행 수` 를 강제한다. 2026-09-20 에 세웠고, 계기는 ①②만
  검토한 세 행을 고친 패스가 **바로 다음 창에서 그대로 재발**한 것이다.
  - **지문에 묶는 이유**: 이 원장의 재판정 관례는 결과 칸을 덮어쓰지 않고 옛 문면을 접미사로
    남기는 것이다. 토큰의 *존재만* 세면 어제 59줄에서 내린 판정이 오늘 79줄이 된 같은 행을 계속
    「판정 완료」로 인증해 **묻지 않은 줄이 기계 초록을 얻는다.**
  - **이 수는 단조 감소가 아니다** — 재판정은 그 행을 미판정으로 되돌려 잔량을 올린다. R9 가
    지키는 불변식은 「줄어들기만 한다」가 아니라 **「판정하지 않은 내용이 판정 완료로 계수되지
    않는다」** 다(R4·R8 의 잔량과 다른 지점).
  - **R9 도 「이 주석이 중복인가」는 판정하지 않는다** — 그 판정은 위 판정 절차대로 사람이 하고,
    R9 가 막는 것은 *묻지 않은 것이 조용히 착지하는 것* 하나다.
- **그 래칫은 docstring 표면에도 걸려 있다**(R10, 2026-09-20): R9 가 설 때 덮은 것은 줄 주석
  표면 하나여서 docstring 23행 / 1665줄은 ③④ 를 한 번도 묻지 않은 채 남았고, 그 잔량을
  세는 마커가 없어 **다시 묻게 될 계기 자체가 없었다**(줄 주석 쪽은 `복원경로-잔량` 마커가
  움직이면 재감지가 열리지만 docstring 쪽은 그 경로가 0 이었다). R10 은 같은 판정을 같은
  함수(`check_paths34()`)로 docstring 원장에 돌리고 잔량만 `docstring-복원경로-잔량` 마커로
  갈라 읽는다.
  - **규칙은 공유하고 마커는 가른다.** 규칙을 복사해 두 벌로 두면 한쪽만 조용히 느슨해지는
    경로가 남는다 — R9 가 한 표면에만 섰던 것이 정확히 그 상태다. 반대로 잔량 마커를 하나로
    합치면 **어느 표면이 움직였는지가 diff 에서 사라지고** 한쪽의 판정이 다른 쪽의 미판정을
    가린다(R4 와 R8 을 갈라 둔 것과 같은 근거).
- **①② 축에도 같은 래칫이 걸려 있다**(R11·R12, 2026-09-25): R9·R10 이 닫은 것은 ③④ 축 하나이고,
  ①②(코드 자체 · 저장소 문서)를 물었다는 주장에는 기계 근거가 없어 그 축의 판정은 **다시 물게 될
  계기 없이 낡았다.** R11(줄 주석)·R12(docstring)는 결과 칸이 `복원경로 ①:`·`복원경로 ②:` 를
  **둘 다** 담고 「복원경로 ①② 판정 @지문 \<12자\>」 의 지문이 그 행의 현재 지문과 같을 때만
  완료로 세며, 잔량은 `복원경로①②-잔량`·`docstring-복원경로①②-잔량` 마커로 읽는다.
  - **계기가 된 실측 둘**: 원장 docstring 행 `#5` 의 보존된 ①② 판정은 「복원 경로 넷 어디에도
    없다」고 단정한 **여섯 자리 중 넷이 저작 PR 본문에 있었고**, 행 `#6` 의 2026-09-07 ①② 판정은
    **다섯 자리 전부에서 어긋난 채 18일간 초록으로 통과**했다. 두 경우 다 ③④ 앵커는 초록이었다 —
    **한 축의 완료가 다른 축의 미판정을 가린다.**
  - **네 자리가 한 함수를 쓴다.** `check_paths()` 가 축(`tokens`·`anchor_re`)과 표면을 인자로 받아
    R9~R12 네 조합을 돌린다(2026-09-20 까지의 이름은 `check_paths34()` 였다).
  - **초기값은 「거의 전부 미판정」이고 후퇴가 아니다** — 줄 주석 67행 전부와 docstring 31행이
    잔량으로 선언된다. 지금까지 세지 않았던 것을 처음 세는 값이다.
...
| 사각지대 | 현재 | 근거 |
| --- | --- | --- |
| **Python docstring 본문** | 게이트가 **별도 표면으로 본다**(`ledger.md`의 「docstring 판정 이력」). 2026-09-20 부터 ③④ 판정 래칫(R10)이, 2026-09-25 부터 ①② 판정 래칫(R12)도 이 표면에 걸린다 | 줄 접두사로 식별되지 않아 정규식을 넓혀도 잡히지 않는다 — `ast` 파서가 필요하다 |
