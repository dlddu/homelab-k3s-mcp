...
- **판정이 *무엇을 물었는지*에도 래칫이 걸려 있다**(R9): 위 여덟 규칙은 등재 범위가 *변했는지*만
  ...
  rc=0 으로 착지한다. R9 는 결과 칸이 `복원경로 ③:`·`복원경로 ④:` 를 **둘 다** 담고
...
| 사각지대 | 현재 | 근거 |
| --- | --- | --- |
| **Python docstring 본문** | 게이트가 **별도 표면으로 본다**(`ledger.md`의 「docstring 판정 이력」). 2026-09-20 부터 ③④ 판정 래칫(R10)도 이 표면에 걸린다 | 줄 접두사로 식별되지 않아 정규식을 넓혀도 잡히지 않는다 — `ast` 파서가 필요하다 |
