...
- **판정이 *무엇을 물었는지*에도 래칫이 걸려 있다**(R9): 위 여덟 규칙은 등재 범위가 *변했는지*만
  ...
  rc=0 으로 착지한다. R9 는 원장 행의 결과 칸이 `복원경로 ③:` 와 `복원경로 ④:` 를 **둘 다**
  담는 행만 ③④ 판정 완료로 세고 `완료 행 수 + 잔량 == 전체 행 수` 를 강제한다 — 적지 않은 행이
  들어오면 잔량을 올리는 명시 선언 없이는 CI 가 멈추고, 그 면제 면적은 줄어들기만 한다.
  2026-09-19 에 세웠고, 계기는 ①②만 검토한 세 행을 고친 패스가 **바로 다음 창의 다섯 행에서
  그대로 재발**한 것이다(`ledger.md`의 「③④ 잔량 래칫」 경위 참조). **R9 도 「이 주석이 중복인가」는
  판정하지 않는다** — 그 판정은 위 판정 절차대로 사람이 하고, R9 가 막는 것은 *묻지 않은 것이
  조용히 착지하는 것* 하나다.
