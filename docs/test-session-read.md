...
- **검증 AC**: AC2
- **자동화**: (미작성) 통합
  `tests/integration/session.py::test_session_read_ac2_state_branch_disclosure`
...
- **검증 AC**: AC3
- **자동화**: (미작성) Go 단위 `internal/sessionplatform/sessionplatform_test.go`
  (`TestReadRejectsNegativeOffset`, `TestReadNotFound`) + 통합
...
- **검증 AC**: AC4
- **자동화**: (미작성) 통합
  `tests/integration/no_config.py::test_session_read_ac4_unconfigured_refusal`
