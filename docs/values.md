...
- **현재 상태 (2026-09-21, 같은 날 세 번째 갱신)**: `prd-event-log` **AC1·AC3 에 이어 AC2·AC4 가 착지했다**
  — `internal/eventlog` 의 레코드(`{시각, 도구, 주체, 대상, 결과, 사유, 게이트}`)를 디스패처가 호출당
  한 줄로 stdout 에 낸다. 주체는 JWT `sub` 또는 API 키 위치이고, 좌표 네 필드 외의 값(인자·응답
  본문·자격증명·Secret 데이터·exec 페이로드)은 레코드에 실릴 자리가 없다. 거부 레코드는 `prd-metrics`
  AC2 의 여덟 사유 중 하나를 싣고, 게이트 대상이면 `request_id`·판정·자동 승인 표기를 필드로 싣는다;
  인증 실패·입력 검증·게이트·통합 미설정 거부가 전부 레코드를 남긴다. stdout 너머 보존(AC5)과
  **메트릭 표면 전부**(`prd-metrics`)는 아직 0이다.
