...
- **유형**: 구체적
- **설명**: 서버가 무엇을 실행했고 무엇을 거부했는지가 **사후에 재구성 가능한 기록**으로 남고,
  호출·거부·지연이 수치로 관측된다. V3가 "막는" 가치라면 V6는 **"막았는지, 못 막았는지 알 수
  있는" 가치**다. V3의 보증은 승인 게이트 하나에 전부 걸려 있고 RBAC 백스톱이 없으므로,
  게이트가 오작동해도 apiserver가 막아 주지 않는다 — 그때 남는 마지막 층이 기록이다. 이 가치가
  지향하는 방향은 **사고가 났을 때 "무슨 일이 있었는가"를 대화 기록이 아니라 서버 기록으로
  답하는 것**이다.
- **측정 가능 목표**:
  - 모든 도구 호출에 `{시각, 도구, 호출 주체(JWT `sub` 또는 API 키 식별자), 리소스 좌표,
    결과(성공·거부·에러)}`가 한 레코드로 남는다
  - 게이트 대상 호출은 gatekeeper `request_id`와 판정(승인·거절·만료·타임아웃·통신 실패·미설정)이
    같은 레코드에서 연결된다. `AUTO_APPROVE`로 승인된 호출은 그 사실이 표기된다
    (`prd-approval-gate` AC9가 이미 요구하는 로그 표기와 같은 출처)
  - 메트릭으로 **도구별 호출 수 · 거부 수(사유별) · 처리 지연**을 관측한다
  - **기록에 싣지 않는 것**: 자격증명 값, Secret 본문, `exec`·`proxy`의 페이로드와 응답 본문.
    기록이 V2가 좁혀 둔 노출 면을 다시 넓히지 않는다
  - 기록은 **파드 stdout 너머로 보존**된다 — 파드가 재시작·교체돼도 남는다. 수집·보존 경로는
    가치가 정하지 않고 PRD에서 정한다
  - **범위 밖**: 분산 트레이싱(스팬·전파). 필요해지면 별도 판단으로 넓힌다
- **경계**: V4는 **사람이 의도해서 쓰는** 운영 지식이고, V6는 **서버가 자동으로 남기는** 행위
  기록이다. V2의 `grafana_token`은 운영자가 관측 스택에 접근할 자격증명을 내주는 것이지 서버
  자신의 텔레메트리가 아니다.
- **현재 상태 (2026-09-21)**: 씨앗만 있다 — `internal/mcp/gate.go`가 `approval granted`·
  `approval refused`를 `request_id`와 함께, `internal/auth/auth.go`가 인증 방식을,
  `internal/server/server.go`가 HTTP 요청을 각각 `slog`로 남긴다. 그러나 **필드·범위·보존
  어느 것도 계약된 적이 없고 PRD·AC·테스트가 0이다** — 문서 체계 관점에서 V6는 현재
  **PRD 없는 가치**다(`doc-tracker/`의 위험 진단).
...
| 2026-09-21 | **V6(서버 행위의 감사 가능성) 추가** — 기존 다섯 가치 어디에도 "서버가 자기 행위를 남긴다"가 없었다. V3(막는다)·V4(사람이 쓰는 문서)·V2(운영자에게 주는 관측 자격증명)와 각각 경계를 그었다. 범위는 **감사 기록 + 메트릭**이고 분산 트레이싱은 뺐다. 보존처는 가치에서 정하지 않고 「stdout 너머」만 걸어 PRD로 넘긴다. **이 가치는 PRD·AC·테스트가 0인 채로 선다** — 가치가 구현·문서보다 앞서는 첫 사례이며, 그 공백은 `doc-tracker/2026-09.md`의 「PRD 없는 가치」에 위험으로 등재했다. |
