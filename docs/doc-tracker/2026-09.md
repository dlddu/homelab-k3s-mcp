...
|------|-----------|-----------|-----------|
| 2026-09-19 | `rct_20260919-0004` — **주석 판정에 복원 경로 ③④ 래칫(게이트 R9)을 세우고, 창 #146·#147 의 다섯 행을 ③④ 까지 재판정했다.** ⑴ **무엇이 문제였나**: `check_comment_policy.py` 의 R1~R8 은 등재 범위가 *변했는지*만 재고 그 판정이 복원 경로 넷 중 **어디까지 물었는지**는 보지 않는다. 그래서 ①(코드)·②(저장소 문서)만 검토한 판정이 rc=0 으로 착지한다 — 2026-09-19 의 `rct_20260919-0001` 이 그 형태를 세 행에서 찾아 고쳤는데 **바로 다음 창의 다섯 행에서 그대로 재발**했다. ⑵ **세운 것**: R9 — 원장 행의 결과 칸이 `복원경로 ③:`·`복원경로 ④:` 를 둘 다 담는 행만 ③④ 판정 완료로 세고, `완료 행 수 + 잔량 마커 == 전체 행 수` 를 강제한다(R4·R8 과 같은 잔량 래칫). 여는 값은 **44/50 미판정**이다. ⑶ **재판정**: 창의 다섯 행(`e53cbc4c203c` · `766c22d0756d` · `aa46a5d651db` · `92477ed3e3e2` · `3d7f820c8536`)을 ③④ 에 대해 항목 단위로 물었다 — ③ 은 PR #146·#147 본문에서 10항목이 확인되고 ④ 는 두 커밋 모두 본문 비공백 1줄이라 **전건 ❌** 다. 겹치는 항목은 2026-09-14 패스가 그은 경계(「도달 가능성과 집행은 다르다」)가 살아 있어 절차 5번으로 **전건 유지**했다 — **주석 제거 0줄**. ⑷ **함께 정정**: attach 행의 「+39 −4 = +37」(자체 모순 · 실측 +41 −4)과 「이 실측치를 말하는 **유일한** 자리」 주장 — 같은 수치가 `test-resource-generic.md` 시나리오 13 과 이 문서의 같은 행에 이미 있다. 그럼에도 유지인 근거는 **그 ② 표면이 규칙 6 의 「임시」 등재**라 해제 시 철거가 예약돼 있다는 것이고, 지우려면 절차 4번대로 영속 문서로 먼저 옮겨야 한다(그 자리는 자매 모델 `tbm_homelab-k3s-mcp-scenario-e2e` 가 시나리오 13 을 닫으며 정한다). ⑸ **시나리오·AC·PRD·테스트 문서의 신설·삭제·개정 0건**이고 e2e 레지스트리·집계도 불변이다. 새 문서는 판정 상세 `docs/comment-policy/passes/2026-09-19-restoration-paths-3-4-window-146-147.md` 하나이고 허브 집계가 49 → 50(정책 13 → 14)으로 움직인다. | 허브 50 미만 · ③④ 판정 래칫 없음 · 창 5행 ①② 판정 | 허브 50 / 50 · R9 가동(③④ 6/50행 · 잔량 44) · 창 5행 ③④ 판정 완료 |
