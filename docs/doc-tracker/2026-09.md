...
### 시나리오 레지스트리 (85) — ✅ 전용 파일 68 · ⬜ 분할 대기 0 · ⏳ 구현 대기 16 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **85 − 1(예외) − 16(구현 대기) = 68 = 매칭 파일 68**, 공백 **0**.
...
| test-resource-generic.md#시나리오 15 | 프록시는 경로를 숨기지 않는다 | ⏳ 구현 대기 (규칙 6 — 상세 행 참조; `resource_proxy` 는 #150 으로 등록됐다. 남은 차단은 전용 파일 저작뿐이라 이 행의 ⏳ 자격은 재판정 대기이고, 철거는 소관인 `tbm_homelab-k3s-mcp-scenario-e2e` 의 몫이다) |
...
| **test-resource-generic.md#시나리오 15** | `resource_proxy` 가 등록돼 있지 않다(레포 전체 grep 0 히트). `pods/proxy`·`services/proxy`·`nodes/proxy` 는 2026-09-14 RBAC 개정으로 전부 권한 안이다. 네 호출 모두 승인을 요구하는데 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. | docs-impl(도구) → 이 렌즈(e2e) | ⑴ `resource_proxy` 등록 **그리고** ⑵ 실물 gatekeeper 픽스처. HTTP 파드·Service 픽스처는 이 렌즈 자신의 저작이라 차단 요인이 아니다 **⟨2026-09-18 갱신⟩ 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`·`tests/k8s/kind/gatekeeper-variant.yaml`)는 이 PR(#106)로 착지했다 — 픽스처 부재는 더는 이 행의 차단 요인이 아니고, 남은 것은 위에 적힌 나머지 조건과 전용 e2e 파일 저작이다.** **⟨2026-09-19 갱신 · `rct_20260919-0004`⟩ 해제 조건 ⑴(`resource_proxy` 등록)이 이 PR 로 충족됐다** — `internal/k8s/proxy.go`·`internal/mcp/proxy.go`·`gate.go` 의 `resource_proxy` 선언(`resolve: proxyPairs()` · `alwaysGated`)·`toolslist.go`·`_helpers.py::EXPECTED_TOOLS` 다섯 지점 실측 히트. 이 시나리오가 재는 넷 중 **셋은 단위로 이미 섰다**: 메서드→verb 다섯 쌍이 전부 게이트를 타는 것(`GET` 포함 — `gatedVerbs` 가 verb 로만 판정하므로 `toolDeclaration.alwaysGated` 를 더해 열었다), `/healthz` 와 `/exec` 이 **같은 쌍**으로 표현되는데도 승인 `context` 가 경로 전문과 고권한 표시로 둘을 가르는 것, 그 표시가 **막지 않는 것**. ⑵ 실물 픽스처는 #106 으로 이미 닫혔다. 그래서 **남은 차단은 전용 `resource_generic_ac15.py` 저작 하나**이고, 그 파일이 재야 할 것은 단위가 원리적으로 못 보는 자리다 — 실물 승인 화면에 실제로 무엇이 실리는지와, 승인된 `nodes/proxy` 의 `/exec` 이 **정말 실행되는지**(가짜 게이트 위에서는 「실행되되」를 단언할 수 없다). ⚠️ `ag#1` 행에 적은 규칙 4·6 의 배제가 이 행에도 걸린다 — 이 PR 은 상태 표기를 건드리지 않고, 저작과 철거는 `tbm_homelab-k3s-mcp-scenario-e2e` 가 같은 PR 에서 할 것. 집계 무이동. |
