...
- 등록 도구: **22종** (`internal/mcp/toolslist.go` = `tests/integration/_helpers.py::EXPECTED_TOOLS`) — 그중 게이트 대상 쓰기 도구는 `resource_create`·`resource_update`·`resource_patch`·`resource_delete`·`resource_delete_collection`·`resource_exec`·`resource_attach` 7종이고, 게이트 밖 예외는 **1종**(`dear_baby_reset_user`)이다. `resource_watch` 는 쓰기가 아니므로 이 둘 어디에도 들어가지 않는다 — 민감 종류(`RESOURCE_GATED_KINDS`)를 대상으로 할 때만 `resource_get` 과 같은 조건으로 게이트를 탄다
- **문서 선행(미구현)**: `github_commit_status_create`(PRD AC6개)는 2026-09-19 문서만 섰다 — 등록 도구 수 22종에 들어가지 않으며, 그 PRD의 시나리오 6개는 아래 「⏳ 구현 대기」에 등재했다. 클러스터 밖(GitHub) 쓰기이므로 게이트 대상·게이트 밖 예외 어느 계수에도 들어가지 않는다(`prd-github-commit-status` 「범위 밖」). **`github_app_installation_token` AC5(statuses 쓰기 배제 · 읽기는 발급)는 문서 선행에서 빠졌다** — 구현이 `internal/github` 에 착지했다(`rct_20260918-0004`). AC5는 새 도구가 아니라 기존 도구의 발급 규칙이므로 등록 도구 수를 올리지 않았다(21→22 는 이 줄과 무관하게 `resource_delete_collection` 이 올린 것이다 — `rct_20260919-0001`). 이로써 `prd-github-commit-status` 「배포 순서(필수)」의 ①이 닫혔고, 남은 선행은 ②(GitHub UI 에서 App 에 `Commit statuses: Read and write` 추가 — 사람 작업)다
