...
| **test-resource-generic.md#시나리오 14** | ⟨2026-09-19 갱신 · `rct_20260919-0002`⟩ **직전 문면 「`resource_port_forward` 가 등록돼 있지 않다(레포 전체 grep 0 히트)」는 이 PR 로 거짓이 됐다** — `internal/k8s/port_forward.go`·`internal/mcp/port_forward.go`·`gate.go` 의 `create on pods/portforward`·`toolslist.go`·`_helpers.py::EXPECTED_TOOLS` 다섯 지점 실측 히트다. `pods/portforward` 는 2026-09-14 RBAC 개정으로 권한 안이고, 실물 gatekeeper 픽스처는 PR #106 으로 착지했다. **남은 차단은 전용 e2e 저작 하나뿐이고, 그것은 규칙 4 가 등재 사유에서 배제하지 않는 「이 렌즈 소관의 저작」이다.** 「같은 승인 id 로 두 번째 왕복」은 코드 쪽에서 `defer conn.Close()` 가 지지만 그것이 실제로 두 번째 왕복을 막는지는 실물 apiserver 위에서만 관측된다. `pods/portforward` 는 2026-09-14 RBAC 개정으로 권한 안이다. 「같은 승인 id 로 두 번째 왕복」을 재려면 실물 승인이 필요한데 `tests/k8s/kind/gatekeeper-fixture.yaml` 이 없다. | 이 렌즈(e2e) — 전용 `resource_generic_ac14.py` 의 저작 | ⑴ `resource_port_forward` 등록 **2026-09-19 충족**(`rct_20260919-0002`) · ⑵ 실물 gatekeeper 픽스처 **충족**(PR #106). 둘 다 닫혔으므로 남은 것은 저작뿐이다. HTTP 서빙 파드 픽스처는 이 렌즈 자신의 저작이라 차단 요인이 아니다 **⟨2026-09-18 갱신⟩ 실물 gatekeeper 픽스처(`tests/k8s/kind/gatekeeper-fixture.yaml`·`tests/k8s/kind/gatekeeper-variant.yaml`)는 이 PR(#106)로 착지했다 — 픽스처 부재는 더는 이 행의 차단 요인이 아니고, 남은 것은 위에 적힌 나머지 조건과 전용 e2e 파일 저작이다.** |
