...
### 시나리오 레지스트리 (85) — ✅ 전용 파일 66 · ⬜ 분할 대기 0 · ⏳ 구현 대기 18 · 🚫 예외 1

> 불변식이 여기서 눈으로 닫힌다: **85 − 1(예외) − 18(구현 대기) = 66 = 매칭 파일 66**, 공백 **0**.
...
| test-resource-generic.md#시나리오 14 | 포트 포워드는 승인 하나로 세션 하나를 연다 | ⏳ 구현 대기 (규칙 6 — 문서↔구현 불일치) |
...
| **test-resource-generic.md#시나리오 14** | ⟨2026-09-20 갱신⟩ **AC14 가 단발 왕복에서 「승인 하나로 지속 세션 하나」로 개정되어 현 구현이 문서와 어긋난다** — `internal/k8s/port_forward.go` 는 호출마다 `defer conn.Close()` 로 터널을 닫고 `tunnelClosed` 를 상수 `true` 로 싣는 단발 왕복이라, 개정된 시나리오가 요구하는 `sessionId` 반환·같은 세션의 재왕복·유휴/수명 만료·주체 결속이 관측될 대상이 **코드에 없다**. 직전 문면(「남은 차단은 전용 e2e 저작 하나뿐」)은 개정 전 계약 기준이었고 이 개정으로 거짓이 됐다. 실물 gatekeeper 픽스처(PR #106)와 `pods/portforward` 권한(2026-09-14 RBAC 개정)은 여전히 충족 상태다. | 구현(`tbm_homelab-k3s-mcp-docs-impl`) → 이 렌즈(e2e) — 세션 구현 후 전용 `resource_generic_ac14.py` 의 저작 | ⑴ AC14 세션 모델 구현(세션 저장소 · `sessionId` 입력 · `sessionSeconds`·유휴 상한 · 주체 결속 · 송신별 감사 로그 · 승인 `context` 개정) · ⑵ 실물 gatekeeper 픽스처 **충족**(PR #106) · ⑶ 전용 e2e 저작. ⑴ 이 닫히기 전에는 저작해도 관측 대상이 없다 |
...
|------|-----------|-----------|-----------|
| 2026-09-20 | **`resource_port_forward`(AC14)의 승인 단위를 「페이로드 한 번」에서 「지속 세션 하나」로 개정(문서 선행)** — 소유자 판단. 단발 왕복은 여러 번 주고받아야 하는 프로토콜(DB 핸드셰이크·인증 후 질의 등)을 원리적으로 다루지 못해, 그런 용도는 전부 `kubectl port-forward` 로 우회되어 기록이 사라진다. ⑴ **AC7(1승인 1실행)과의 정합**: 쿠버네티스 API 호출은 세션을 여는 `create on pods/portforward` 한 번뿐이므로 AC7 을 개정하지 않았다. 2026-09-12 의 「1승인 1실행이 터널에도 걸린다」는 AC7 의 문언이 아니라 당시의 해석이었고, 그 해석을 이번에 거둔다. ⑵ **대가 — 승인 화면이 페이로드를 다 보여 주지 못한다**: 세션 안의 이후 송신은 `context` 에 오르지 않으므로 운영자는 「그 포트에 그 시간 동안 무엇이든 보낼 수 있는 권한」을 승인한다. 「스트림 넷은 RBAC 로 내용을 가릴 수 없어 게이트가 유일한 방어선」(approval-gate AC3)인 상황에서 게이트가 보는 내용이 줄어드는 것이므로, 완화를 셋 박았다 — 수명 상한(`sessionSeconds` 기본 300·상한 900, 유휴 60초)을 `context` 에 싣는다 · 세션은 한 번만 열리고 닫히면 `sessionId` 를 폐기한다 · 세션 안의 각 송신을 세션을 연 승인 요청 id 와 함께 감사 로그에 남긴다(AC8). ⑶ 함께 고친 자리: `prd-approval-gate` 판정 표의 port_forward 행 · AC3 의 port_forward `context` 구성 · `test-resource-generic` 시나리오 14(제목 변경 — 레지스트리 제목 대조 대상이라 위 1:1 표 행도 같은 커밋에서 갱신). ⚠️ **구현이 문서와 어긋난 상태가 된다** — 현 구현(#146)은 단발 왕복이다. 시나리오 14 의 구현 대기 사유를 「전용 e2e 저작」에서 「문서↔구현 불일치」로 되돌렸고 해제 조건에 세션 모델 구현을 앞세웠다. `internal/`·`tests/` 무변경, AC 총계·시나리오 번호·개수 불변. | AC14 = 단발 왕복(터널 비유지) · 승인 `context` = 포트 + 페이로드 전문 · 시나리오 14 사유 = 전용 e2e 저작 | AC14 = 승인 하나당 지속 세션 하나(수명 상한·1회성·주체 결속·송신별 감사) · 승인 `context` = 파드·포트·수명 상한 + 첫 페이로드 전문 · 시나리오 14 사유 = 문서↔구현 불일치 |
