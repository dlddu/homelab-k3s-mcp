...
| 허브 도달 가능 문서 | ✅ **51 / 51** (가치 1 + PRD 15 + 테스트 15 + 상태 추적 5 + 정책 15), 끊긴 링크 0 |
...
| test-github-commit-status.md#시나리오 3 | 잘못된 입력은 상류에 닿지 않는다 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 저작 대기; 도구 선행은 2026-09-20 충족) |
| test-github-commit-status.md#시나리오 4 | context 네임스페이스 밖은 거부 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 저작 대기; 도구 선행은 2026-09-20 충족) |
| test-github-commit-status.md#시나리오 5 | 미설정 시 도구 에러 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 저작 대기; 도구 선행은 2026-09-20 충족) |
| test-github-commit-status.md#시나리오 6 | 어노테이션 광고 | ⏳ 구현 대기 (규칙 6 — 전용 e2e 저작 대기; 도구 선행은 2026-09-20 충족) |
...
| **test-github-commit-status.md#시나리오 2** | ⑴ 닫힘(위와 같다). 남은 벽은 발급 요청 본문을 읽을 스텁 기록 엔드포인트인데, `/_admin/requests` 는 **이미 있다**(2026-09-20 실측) — 남는 것은 저작뿐이다. | docs-impl(도구) → 이 렌즈(e2e) | 위와 동일 |
| **test-github-commit-status.md#시나리오 3** | ⑴ 닫힘(위와 같다). 요청 카운터도 `/_admin/requests` + `DELETE /_admin/requests` 로 **이미 있다**(2026-09-20 실측). | docs-impl(도구) → 이 렌즈(e2e) | 위와 동일 |
| **test-github-commit-status.md#시나리오 4** | ⑴ 닫힘(위와 같다). (b) 갈래는 `GITHUB_COMMIT_STATUS_CONTEXT_PREFIXES` 만 다른 배포 변형도 요구한다 — 구현은 그 env 를 `FromEnv()` 에서 읽으므로 변형은 env 한 줄이다. | docs-impl(도구) → 이 렌즈(e2e) | 위와 동일 **그리고** 접두사 env 미설정 배포 변형 |
| **test-github-commit-status.md#시나리오 5** | **차단 해소(2026-09-20)** — 도구가 등록됐고 미설정 배포(`auth-fixture.yaml`)도 이미 있다. 남은 것은 저작뿐이다. | docs-impl(도구) → 이 렌즈(e2e) | ~~도구 등록~~ **완료(2026-09-20)** — 전용 파일 저작만 남았다 |
| **test-github-commit-status.md#시나리오 6** | **차단 해소(2026-09-20)** — `tools/list` 가 이 도구를 AC6 의 세 값으로 광고한다(Go 단위 `TestToolsListAdvertisesCommitStatus` 가 단언). | docs-impl(도구) → 이 렌즈(e2e) | ~~도구 등록~~ **완료(2026-09-20)** — 전용 파일 저작만 남았다 |
