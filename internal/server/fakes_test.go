...
type fakeK8s struct {
	...
	deleteCollection []deleteCollectionCall
	execCalls        []execCall
...
func (f *fakeK8s) DeleteCollection(_ context.Context, ref k8s.DeleteCollectionRef) (*k8s.DeleteCollectionResult, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.deleteCollection = append(f.deleteCollection, deleteCollectionCall{ref: ref})
