...
이 줄은 두 가지를 주장한다 — ⑴ `docs/` 의 마크다운 문서가 **전부** 허브(`index.html`)에서
도달 가능하고 ⑵ 그 수가 N 이며 종류별 내역이 저렇다는 것. 같은 문서의 「대기·주의 항목」은
이 줄이 어긋나는 원인을 하나로 지목한다: **문서를 추가하면서 허브 집계를 함께 올리지 않는 것.**
...
재도출한 값과 대조한다.** 클러스터도 서드파티 의존성도 필요 없다(표준 라이브러리 전용) — CI 의
lint 잡에서 돈다.
...
#: 허브가 뷰어를 통해 여는 문서. `reader.html?doc=<docs 기준 상대경로>.md` 규약은
#: doc-tracker 의 「이 레포에 맞춘 뷰어 규약」 절이 정의한다.
HUB_LINK_RE = re.compile(r'href="reader\.html\?doc=([^"#]+\.md)')
#: 마크다운 인라인 링크의 `.md` 대상. 앵커(`#...`)와 제목은 떼고 경로만 딴다.
MD_LINK_RE = re.compile(r"\]\(\s*<?([^)\s<>]+\.md)")
#: 집계 줄. 형태가 바뀌면 파싱 실패로 멈춘다(조용히 0 으로 떨어지지 않게).
CLAIM_RE = re.compile(
    r"^\|[ \t]*허브 도달 가능 문서[ \t]*\|.*?\*\*(\d+)[ \t]*/[ \t]*(\d+)\*\*[ \t]*"
    r"\(가치[ \t]*(\d+)[ \t]*\+[ \t]*PRD[ \t]*(\d+)[ \t]*\+[ \t]*테스트[ \t]*(\d+)"
    r"[ \t]*\+[ \t]*상태 추적[ \t]*(\d+)[ \t]*\+[ \t]*정책[ \t]*(\d+)\)[ \t]*,"
...
#: 종류 판정. 인벤토리 표(「문서 인벤토리」 절)가 말하는 다섯 종류와 같은 구분이다.
